...
SUAQ_waypoints_11_20 <-
  SUAQ_waypoints_11_20 %>% mutate(
    ...
    PtType = ifelse((sym == "Block, Blue" |
                    ...
                    "nightnest",
                    PtType),
    PtType = ifelse(grepl("lch", name), # Metainformation for some lch and lcg are not considered because there are only written down for very less points(10-20 points). Information about call direction is given in the activity table
                    "long call heard",
                    PtType),
    PtType = ifelse(grepl("lcg", name),
                    "long call given",
                    PtType),
    PtType = ifelse(grepl("dna", name),
                    "DNA sample taken",
                    PtType)
)
...
tmp <- SUAQ_waypoints_11_20 %>% filter(grepl("dna", name))
grepl("dna", "name lch")
