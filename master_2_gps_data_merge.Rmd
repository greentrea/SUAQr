...
# 2nd extract the 5 attributes from filename: filedate, orangutanname, infant name, followername, followtype. "Adress" in the first field is left in the list to identify the start of a new list. It will be cut out in the seperation of trackpoints and waypoints.
SUAQ_data <-
  SUAQ_data %>% mutate(
...
# 4. Create file with waypoints and file with trackpoints (device)
for (n in 1:length(levels(as.factor(SUAQ_data$fileN)))){
  print(paste("loading track number:",n))
  #Subset the data by MP
  tmp <- subset(SUAQ_data,SUAQ_data$fileN==levels(as.factor(SUAQ_data$fileN))[n])
  tmp1 <- tmp
  if (n==1){
    waypoints<- tmp %>% slice(1:(which(grepl('Address', tmp$ID))-1))
    trackpoints <- tmp %>% slice(which(grepl('trksegID', tmp$ID))+1:n())
    colnames(trackpoints) <-tmp %>%filter(.$lat=='trksegID')
  }
  else{
    waypoints_tmp<- tmp %>% slice(1:(which(grepl('Address', tmp$ID))-1))
    trackpoints_tmp<-tmp<-tmp %>% slice(which(grepl('trksegID', tmp$ID))+1:n())
...
#to check: How many waypoints per follow (filename), see if read in was correct with original files? # is correct!!
tmp_files<- waypoints %>% group_by(fileN) %>% summarize(count=n())

# 5. get rid of empty collumns
waypoints <- waypoints[,colSums(is.na(waypoints))<nrow(waypoints)]
trackpoints <- trackpoints[,colSums(is.na(trackpoints))<nrow(trackpoints)]


# 6. Extracting information from Name field: PtType--> Type of point: Longcall, Nest etc. See GPS processing information sheet. 
waypoints <-
  waypoints %>% 
  mutate(
    actualTime=time,
