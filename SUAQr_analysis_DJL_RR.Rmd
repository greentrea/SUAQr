...
Tree_follows <- Tree_follows %>% 
  ...
  mutate(tree_RR=tree_DJL/tree_TDD) %>%
  ungroup() %>% 
...
### Full model mit NA's
glm_full <-lmer(DJL ~ rain_eve + walevel_eve + rain_mo + temp_min_eve + temp_min_mo + avg_temp_tot  + fai +ClassFocal+ ageOfCurrentOffspring + (1 | monthYearDate)
...
### FRUITTREECOUNT: Full model mit NA's
DJL_follows_naomit_trees <- DJL_follows %>% dplyr::select(ageFromDOB,tree_RR,tree_DJL,rain_eve, walevel_eve, rain_mo, temp_min_eve, temp_min_mo, avg_temp_tot, ClassFocal, fai, ageOfCurrentOffspring, monthYearDate,focal,meanTimelag,numOfPoints,fruittreecount) %>% na.omit()

glm_full_fruittrees <-lmer(fruittreecount ~ rain_eve + walevel_eve + rain_mo + temp_min_eve + temp_min_mo + avg_temp_tot  + fai + ageFromDOB+ageOfCurrentOffspring  + (1 | monthYearDate)
                    + (1 | meanTimelag), # random effects to correct for
  data = DJL_follows_naomit_trees
  )
glm_significant_fruittrees <-lmer(fruittreecount ~ avg_temp_tot + ageFromDOB  + (1 | monthYearDate)
                    + (1 | meanTimelag), # random effects to correct for
  data = DJL_follows_naomit_trees
  )
glm_randomeffects_fruittrees <- lmer(fruittreecount~(1 | monthYearDate)+ (1 | meanTimelag),data=DJL_follows_naomit_trees)
...
glm_full_DJLtree <-lmer(tree_DJL ~ rain_eve + walevel_eve + rain_mo + temp_min_eve + temp_min_mo + avg_temp_tot  + fai + ageFromDOB+ageOfCurrentOffspring  + (1 | monthYearDate)
                    + (1 | meanTimelag), # random effects to correct for
  data = DJL_follows_naomit_trees
  )
glm_randomeffects_DJLtree <- lmer(tree_DJL~(1 | monthYearDate)+ (1 | meanTimelag),data=DJL_follows_naomit_trees)

anova(glm_full_DJLtree,glm_significant_DJLtree,glm_randomeffects_DJLtree)
...
glm_full_RRtree <-lmer(tree_RR ~ rain_eve + walevel_eve + rain_mo + temp_min_eve + temp_min_mo + avg_temp_tot  + fai + ageFromDOB+ageOfCurrentOffspring  + (1 | monthYearDate)
                    + (1 | meanTimelag), # random effects to correct for
  data = DJL_follows_naomit_trees
  )
glm_randomeffects_RRtree <- lmer(tree_RR~(1 | monthYearDate)+ (1 | meanTimelag),data=DJL_follows_naomit_trees)

anova(glm_full_RRtree,glm_randomeffects_RRtree)
drop1(glm_full_RRtree)
vif.mer(glm_full_RRtree)
