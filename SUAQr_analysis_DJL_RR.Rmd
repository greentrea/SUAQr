...
colnames(DJL_follows) <- c("follow","focal","ClassFocal","rain_eve","watlevel_mo","walevel_eve","rain_mo","avg_temp_tot","max_temp_average","min_temp_average","temp_max_mo","temp_min_mo","temp_max_eve","temp_min_eve","fai","ageOfCurrentOffspring","monthYearDate","DJL","count","turningAngleSum","meanTimelag","medianTimelag")
...
### Basic model
lm_fullbasic <-lm(DJL ~ rain_eve + rain_mo + watlevel_mo + walevel_eve + max_temp_average + min_temp_average 
                    +temp_max_mo+temp_min_mo+avg_temp_tot + ClassFocal + fai +ageOfCurrentOffspring , # random effects to correct for
  data = DJL_follows
  )
autoplot(lm_fullbasic)


### Full model
glm_full <-lmer(DJL ~ rain_eve + rain_mo + watlevel_mo + walevel_eve + max_temp_average + min_temp_average 
...
SUAQ_move_ellie <- SUAQ_rangepoints_11all20_loc[order(SUAQ_rangepoints_11all20_loc$focal,SUAQ_rangepoints_11all20_loc$manualDatetime , decreasing = FALSE ),] %>% filter(focal=="ellie") %>% ungroup()
...
SUAQ_telemetry<- as.telemetry(SUAQ_move_ellie)
SUAQ_telemetry
SVF <- variogram(SUAQ_telemetry)
...
SUAQ_rangepoints_11all20_loc <- SUAQ_rangepoints_11all20_loc[with(SUAQ_rangepoints_11all20_loc, order(focal,manualDatetime)),]

SUAQ_rangepoints_11all20_loc_2<- SUAQ_rangepoints_11all20_loc[order(SUAQ_rangepoints_11all20_loc$focal,SUAQ_rangepoints_11all20_loc$manualDatetime , decreasing = TRUE ),] 
SUAQ_move_2 <- move(x=SUAQ_rangepoints_11all20_loc$E, y=SUAQ_rangepoints_11all20_loc$N, 
...
corridor<- corridor(SUAQ_move_2)


SUAQ_move <- spTransform(SUAQ_move_2,CRSobj="+proj=longlat +datum=WGS84 +no_defs")
