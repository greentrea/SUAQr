...
##### Scatters #####
test <- DJL_follows %>% ungroup() %>% dplyr::select(follow,ClassFocal,rain_eve,watlevel_mo,walevel_eve,rain_mo,avg_temp_tot,max_temp_average,min_temp_average,temp_max_mo,temp_min_mo,fai,ageOfCurrentOffspring,DJL,TDD)
test%>% GGally::ggpairs(echo=FALSE, message=FALSE)
...
plot_gps_pttype<-SUAQ_waypoints_11all20_loc[order(SUAQ_waypoints_11all20_loc$manualTime , decreasing = TRUE ),] %>% filter(PtType!="abnormal off point") %>% 
