...
```
#### Script description
This is the main script for calculating movement parameters on daily or hourly level (level 3). For example day journey length straightness index or sinuosity index.
...
packages_checker(
  c('sjlabelled',
    'sjmisc',
    'sjstats', # further statistics : infos:https://bookdown.org/animestina/phd_july_19/simple-linear-model-and-mixed-methods.html#multilevel-modelling-with-random-intercepts-and-slopes
...
#### Adding some explanatory variables
...
colnames(DJL_follows_noNN) <- c("follow","followtype","focal","ageFromDOB","ClassFocal","rain_eve","watlevel_mo","walevel_eve","rain_mo","avg_temp_tot","max_temp_average","min_temp_average","temp_max_mo","temp_min_mo","temp_max_eve","temp_min_eve","tot_rain_before","mmax_temp_before","fai","ageOfCurrentOffspring","monthYearDate","numOfPoints","DJL","count","turningAngleSum","meanTimelag","medianTimelag","original_followlength","gps_followlength","speed")
...
write_csv(DJL_follows_noNN, path = paste(
  paste(
...
write_csv(DJL_follows, path = paste(
  paste(
    "/Users/stefgr/Nextcloud/UZH/12_Semester/20200313_Masterthesis/R/data/Processed_data/",
    "DJL_follows",
    sep = ""
...
SUAQ_weather_fai_overyears<- SUAQ_weather_fai_soi_monthly %>%  ggplot(aes(yearmonth,group = SUAQ_weather.year))+
  ...
  labs(x="time [year]",y="fruit availability     /      temprature [°C]")+
  scale_colour_manual("",
...
for(i in random_window_sizes){
  ...
  points_available_start<- dplyr::setdiff(points_available,points_available_start_anti)
  ...
  test <- points_available %>% filter(follow%in%c(random_start$follow))
...
### renaming and selecting for stats analysis 2
...
plot_Betweentreedirectness_test<- SUAQ_treedirectness %>% 
  ggplot( aes(x=tree_betweennumgps, y=tree_betweendirectness)) +
    scale_fill_viridis(discrete = TRUE, alpha=0.6) +
    geom_jitter(color="blue", size=1, alpha=0.2) +
    geom_jitter(data=SUAQ_randomdirectness_stats,aes(random_betweennumgps,random_betweendirectness),color="red", size=1, alpha=0.2) +
    theme_ipsum() +
    theme(
      #legend.position="none",
      plot.title = element_text(size=11)
    ) +
    labs(x="Windowsize number of fixes between trees",y="SI")+
  ylim(0,1)+
  geom_smooth(data=SUAQ_randomdirectness_stats,aes(sqrt(random_betweentravelled),random_sinuosity),color="red",method="loess")+
  geom_smooth(method="loess")
plot_Betweentreedirectness_test
...
### Statistics
# Simple statistics
sample_SUAQ_treedirectness <- SUAQ_treedirectness %>% filter(tree_betweennumgps==5) %>% mutate(sample = "between trees")
sample_SUAQ_randomdirectness <- SUAQ_randomdirectness %>% filter(random_betweennumgps==5)%>% mutate(sample = "random") 

sample_SUAQ_treedirectness %>% ggplot(aes(x=tree_betweendirectness,y=sample))+geom_boxplot()+geom_boxplot(data= sample_SUAQ_randomdirectness,aes(x=random_betweendirectness))
...
SUAQ_treedirectness_stats_naomit_mother <- SUAQ_treedirectness_stats %>% left_join(explanatoryvars[,c(1,3:4)],by=c("focal"="focal"))%>%  dplyr::select(focal,tree_betweennumgps,ClassFocal,dominance,manualDate,tree_betweendirectness,tree_sinuosity,rain_eve,walevel_eve,rain_mo,temp_min_eve,temp_min_mo,avg_temp_tot,fai,ageOfCurrentOffspring,matriline,ageFromDOB,tree_betweentime,rainyseason,tree_betweentravelled) %>% na.omit()
...
glm_SIN_FTD_mothers <- lmer(tree_sinuosity~rain_eve+rain_mo+avg_temp_tot+fai+ dominance+ageOfCurrentOffspring+matriline+(1|focal) +(1 | tree_betweennumgps)+ (1 | manualDate)+(1|tree_betweentime),data=SUAQ_treedirectness_stats_naomit_mother)
...
#### Plot and tables
ggpairs(SUAQ_treedirectness_stats_naomit[,c(0:13,15:16)])
...
summary(glm_SIN_FTD_mothers)
sjPlot::tab_model(glm_SIN_FTD_mothers,  digits = 4,
                  show.re.var= TRUE, 
                  pred.labels =c("intercept","day rain", "night rain","average temperature", "FAI", "Class: flanged male", "Class: infant","Class: juvenile","Class: unflanged male","age","age of offspring","matriline"),
                  dv.labels= "Effects of internal and external factors on DJL")
ranova(glm_SIN_FTD_mothers)
view(as.data.frame(vif.mer(glm_SIN_FTD_mothers)))
r.squaredGLMM(glm_SIN_FTD_mothers)
anova(glm_SIN_FTD_mothers_empty,glm_SIN_FTD_mothers)
...
plot_gps_pttype<-SUAQ_randomdirectness[order(SUAQ_treedirectness$manualDatetime , decreasing = FALSE ),] %>%
  filter(focal %in% c("cissy")) %>%
  group_by(follow) %>%
  ggplot()+
...
ggsave(filename = paste(
  ...
  "png",
  sep = "."
), plot = plot_DOMINANCExFTC)

### Expected meaningless tree to tree follows analysis
### Sinuosity and DJL
plot_DOMINANCExDJL <-
  DJL_follows %>% left_join(explanatoryvars,by=c("focal"="focal")) %>% 
  ggplot(aes(dominance,DJL,group=dominance)) +
  #scale_y_log10(labels=fancy_scientific) +
  geom_boxplot() + 
  labs(x = "Dominance category", y = "DJL [m]")
plot_DOMINANCExDJL

plot_STARTTIMExFAI <-
  SUAQ_rangepoints_11all20_loc %>% 
  group_by(focal,follow,SUAQ_followlog.StartFollow,SUAQ_fai.fai) %>% 
  summarise() %>% na.omit() %>% 
  ggplot(aes(SUAQ_followlog.StartFollow,SUAQ_fai.fai)) +
  #scale_y_log10(labels=fancy_scientific) +
  geom_point() + 
  geom_smooth()+
  labs(x = "Dominance category", y = "DJL [m]")
...
plot_DOMINANCExRR <-
  DJL_follows %>% left_join(explanatoryvars,by=c("focal"="focal")) %>% 
  ggplot(aes(dominance,RR,group=dominance)) +
  #scale_y_log10(labels=fancy_scientific) +
  geom_boxplot() + 
  labs(x = "Dominance category", y = "SI")
plot_DOMINANCExRR

plot_DOMINANCExSIN <-
  DJL_follows %>% left_join(explanatoryvars,by=c("focal"="focal")) %>% 
  ggplot(aes(dominance,trj_sinuosity,group=dominance)) +
  #scale_y_log10(labels=fancy_scientific) +
  geom_boxplot() + 
  labs(x = "Dominance category", y = "SIN")
plot_DOMINANCExSIN

plot_DOMINANCExFTC <-
  DJL_follows %>% left_join(explanatoryvars,by=c("focal"="focal")) %>% 
  ggplot(aes(dominance,fruittreecount,group=dominance)) +
  #scale_y_log10(labels=fancy_scientific) +
  geom_boxplot() + 
  labs(x = "Dominance category", y = "fruit trees")
plot_DOMINANCExFTC
...
### DJL and sinuosity
plot_RRxAge<-
  DJL_follows %>% ggplot(aes(ageFromDOB,RR)) +
  #scale_y_log10(labels=fancy_scientific) +
  geom_point(binwidth = 1) + labs(y = "SI", x = "Age")+
  geom_smooth()
plot_RRxAge
...
### DJL and RR
plot_MINTEMPxRR <-
  DJL_follows %>% ggplot(aes(temp_min_eve,RR)) +
  #scale_y_log10(labels=fancy_scientific) +
  geom_point(binwidth = 1) + labs(x = "minimum temperature [C°]", y = "SI")+
  geom_smooth()
plot_MINTEMPxRR
...
plot_AgeFocalxDJL <-
  DJL_follows  %>%  ggplot(aes(ageFromDOB,DJL,colour=ClassFocal,group=ClassFocal)) +
  #scale_y_log10(labels=fancy_scientific) +
  geom_point() + labs(title = "Scatterplot of DJL and Age of focal", x = "Approximate Age [years]", y = "DJL [m]")+
  geom_smooth(se=FALSE,colour="black")
plot_AgeFocalxDJL

plot_AgeFocalxSIN <-
  DJL_follows  %>%  ggplot(aes(ageFromDOB,trj_sinuosity,colour=ClassFocal,group=ClassFocal)) +
  #scale_y_log10(labels=fancy_scientific) +
  geom_point() + labs(title = "Scatterplot of DJL and Age of focal", x = "Approximate Age [years]", y = "sinuosity")+
  geom_smooth(se=FALSE,colour="black")
plot_AgeFocalxSIN
...
### SIN influenced by class? Yes
kruskal.test(trj_sinuosity~ClassFocal,data = DJL_follows)
DJL_follows %>% ggplot(aes(ClassFocal,meanTimelag))+geom_boxplot()
pairwise.t.test(DJL_follows$trj_sinuosity,DJL_follows$ClassFocal,data = DJL_follows)
DJL_follows %>% ggplot(aes(ClassFocal,trj_sinuosity))+geom_boxplot()
...
hist(SUAQ_rangepoints_11all20_loc$turningAngle)

### SIN influenced by class?
pairwise.t.test(DJL_follows$trj_sinuosity,DJL_follows$ClassFocal,data = DJL_follows)
...
DJL_follows_naomit_ageANDdominance<- DJL_follows %>% dplyr::select(DJL,rain_eve, walevel_eve, rain_mo, temp_min_eve, temp_min_mo,temp_max_eve, temp_max_mo, avg_temp_tot, ClassFocal, fai, monthYearDate,focal,meanTimelag,numOfPoints,ageOfCurrentOffspring,fruittreecount,monthname,rainyseason,ageFromDOB,tot_rain_before,mmax_temp_before,month)  %>%  na.omit()
DJL_follows_naomit_mother<- DJL_follows %>% dplyr::select(DJL,rain_eve, walevel_eve, rain_mo, temp_min_eve, temp_min_mo,temp_max_eve, temp_max_mo, avg_temp_tot, ClassFocal, fai, monthYearDate,focal,meanTimelag,numOfPoints,fruittreecount, ageOfCurrentOffspring,monthname,rainyseason,tot_rain_before,mmax_temp_before,month,ageFromDOB)%>% left_join(explanatoryvars,by=c("focal"="focal")) %>% na.omit()


test<- aov(DJL~dominance,data=DJL_follows_naomit_mother)
summary(test)
...
glm_DJL_selected <-lmer(DJL ~ rain_eve +rain_mo  +  avg_temp_tot   + fai + ClassFocal + meanTimelag+(1 | focal) + (1 | monthYearDate) , # random effects to correct for
  data = DJL_follows_naomit_full
  )
...
summary(glm_DJL_age)
glm_DJL_ageANDdominance <-lmer(DJL ~  rain_eve + walevel_eve +rain_mo +  avg_temp_tot   + fai + ClassFocal + meanTimelag+dominance+ (1 | monthYearDate) + (1 | focal), # random effects to correct for
  data = DJL_follows_naomit_ageANDdominance
  )
summary(glm_DJL_ageANDdominance)
vif.mer(glm_DJL_ageANDdominance)
...
glm_DJL_females_empty <-lmer(DJL ~dominance+ (1 | monthYearDate) + (1 | focal), # random effects to correct for
  data = DJL_follows_naomit_mother
  )
glm_DJL_females_selected <-lmer(DJL ~ rain_eve  + rain_mo  +  avg_temp_tot   + fai+ageOfCurrentOffspring+dominance  + (1 | monthYearDate) + (1 | focal), # random effects to correct for
  data = DJL_follows_naomit_mother
  )
...
###### OVERVIEW OF MODEL RESULTS
screenreg(list(lmm_DJL_focalANDmonthyear,glm_DJL_full,glm_DJL_selected,glm_DJL_females_empty,glm_DJL_females_selected,glm_DJL_ageANDdominance),digits=5)
qqnorm(residuals(glm_DJL_selected))
sjPlot::tab_model(glm_DJL_selected, 
                  show.re.var= TRUE, 
                  pred.labels =c("intercept","day rain","waterlevel evening", "night rain","minimum °C evening","minimum °C morning","maximum °C evening","maximum °C morning","average temperature", "FAI", "Class: flanged male", "Class: infant","Class: juvenile","Class: unflanged male","mean timelag","age of offspring","age"),
                  dv.labels= "Effects of internal and external factors on DJL")
ranova(glm_DJL_females_selected)
view(as.data.frame(vif.mer(glm_DJL_females_selected)))
anova(lmm_DJL_focalANDmonthyear,glm_DJL_selected)

###### TEXTREG --> LATEX EXPORT
r.squaredGLMM(glm_DJL_selected)
stargazer(glm_DJL_selected)
stargazer(glm_DJL_selected, type = "html", style = "qje")
r.squaredGLMM(glm_DJL_females_selected)
summary(glm_DJL_females_selected)
ranova(glm_DJL_selected)
texreg(list(lmm_DJL_focalANDmonthyear,glm_DJL_full,glm_DJL_selected,glm_DJL_females_empty,glm_DJL_females_selected,glm_DJL_ageANDdominance),
          custom.model.names = c("Only random","Full","Selected","Only random females","Selected only females","Only females with DOB"),
          ...
          use.packages=FALSE,
          single.row=TRUE)
test<- SUAQ_rangepoints_11all20_loc %>% filter(PtType!="tree")
test2<- SUAQ_rangepoints_11all20_loc %>% filter(PtType!="tree") %>% filter(timelag==1800)
...
RR_follows_naomit_mother<- DJL_follows %>% left_join(explanatoryvars,by=c("focal"="focal")) %>%  dplyr::select(RR,dominance,rain_eve, walevel_eve, rain_mo, temp_min_eve, temp_min_mo,temp_max_eve, temp_max_mo, avg_temp_tot, ClassFocal, fai, monthYearDate,focal,meanTimelag,numOfPoints,fruittreecount,ageFromDOB, ageOfCurrentOffspring,monthname,rainyseason,) %>% na.omit()
...
summary(glm_RR_full)
glm_RR_selected <-lmer(RR ~ rain_eve  + rain_mo +  avg_temp_tot   + fai + ClassFocal  +  (1 | monthYearDate) +(1 | focal), # random effects to correct for
  data = RR_follows_naomit_full
  )
...
glm_RR_females_selected <-lmer(RR ~ rain_eve  + rain_mo  +  avg_temp_tot   + fai+ageOfCurrentOffspring+dominance  + (1 | monthYearDate) + (1 | focal), # random effects to correct for
  data = RR_follows_naomit_mother
  )
...
summary(glm_RR_females_selected)
sjPlot::tab_model(glm_RR_females_selected, 
                  show.re.var= TRUE, 
                  pred.labels =c("intercept","day rain","waterlevel evening", "night rain","minimum °C evening","minimum °C morning","maximum °C evening","maximum °C morning","average temperature", "FAI", "Class: flanged male", "Class: infant","Class: juvenile","Class: unflanged male","mean timelag","age of offspring","age"),
                  dv.labels= "Effects of internal and external factors on DJL")
ranova(glm_RR_females_selected)
view(as.data.frame(vif.mer(glm_RR_females_selected)))
r.squaredGLMM(glm_RR_females_selected)
anova(lmm_RR_focalANDmonthyear,glm_RR_females_selected)
?r.squaredGLMM
###### TEXTREG --> LATEX EXPORT
...
TDD_follows_naomit_mother<- DJL_follows %>% left_join(explanatoryvars,by=c("focal"="focal")) %>% dplyr::select(TDD,rain_eve, walevel_eve, rain_mo,dominance, temp_min_eve, temp_min_mo,temp_max_eve, temp_max_mo, avg_temp_tot, ClassFocal, fai, monthYearDate,focal,meanTimelag,numOfPoints,fruittreecount,ageFromDOB, ageOfCurrentOffspring,monthname,rainyseason,) %>% na.omit()
...
glm_TDD_females_selected <-lmer(TDD ~ rain_eve  + rain_mo  +  avg_temp_tot   + fai+ageOfCurrentOffspring+dominance  + (1 | monthYearDate) + (1 | focal), # random effects to correct for
  data = TDD_follows_naomit_mother
  )
...
summary(glm_TDD_females_selected)
sjPlot::tab_model(glm_TDD_females_selected, 
                  show.re.var= TRUE, 
                  pred.labels =c("intercept","day rain","waterlevel evening", "night rain","minimum °C evening","minimum °C morning","maximum °C evening","maximum °C morning","average temperature", "FAI", "Class: flanged male", "Class: infant","Class: juvenile","Class: unflanged male","mean timelag","age of offspring","age"),
                  dv.labels= "Effects of internal and external factors on DJL")
ranova(glm_TDD_females_selected)
view(as.data.frame(vif.mer(glm_TDD_females_selected)))
r.squaredGLMM(glm_TDD_females_selected)
anova(lmm_TDD_focalANDmonthyear,glm_TDD_females_selected)
...
SIN_follows_naomit_dominanceANDageDOP<- DJL_follows %>% left_join(explanatoryvars,by=c("focal"="focal")) %>% dplyr::select(DJL,follow,trj_sinuosity,rain_eve,dominance,ageFromDOB, walevel_eve, rain_mo, temp_min_eve, temp_min_mo,temp_max_eve, temp_max_mo, avg_temp_tot, ClassFocal, fai, monthYearDate,focal,meanTimelag,numOfPoints,fruittreecount,monthname,rainyseason,month,RR) %>% na.omit() 
SIN_follows_naomit_mother<- DJL_follows%>% left_join(explanatoryvars,by=c("focal"="focal")) %>% dplyr::select(DJL,trj_sinuosity,rain_eve, walevel_eve, rain_mo, temp_min_eve, temp_min_mo,temp_max_eve, temp_max_mo, avg_temp_tot,dominance, ClassFocal, fai, monthYearDate,focal,meanTimelag,numOfPoints,fruittreecount,ageFromDOB, ageOfCurrentOffspring,monthname,rainyseason,) %>% na.omit()
...
glm_SIN_selected <-lmer(trj_sinuosity~ rain_eve  + rain_mo +  avg_temp_tot + fruittreecount  + fai + ClassFocal +DJL+ (1 | monthYearDate) +(1 | focal), # random effects to correct for
  data = SIN_follows_naomit_full
  )
glm_SIN_selected_dominance <-lmer(trj_sinuosity~ rain_eve  + rain_mo +  avg_temp_tot   + fai + ClassFocal + DJL+meanTimelag+dominance+ (1 | monthYearDate) +(1 | focal), # random effects to correct for
  data = SIN_follows_naomit_dominanceANDageDOP
  )
summary(glm_SIN_selected_dominance)
glm_SIN_selected_low <-lmer(trj_sinuosity~ fruittreecount + DJL  + (1 | monthYearDate) +(1 | focal), # random effects to correct for
  data = SIN_follows_naomit_full
  )
vif.mer(glm_SIN_selected_low)
...
glm_SIN_females_selected <-lmer(trj_sinuosity~ rain_eve  + rain_mo  +  avg_temp_tot   + fai+ageOfCurrentOffspring+dominance+fruittreecount +DJL + (1 | monthYearDate) + (1 | focal), # random effects to correct for
  data = SIN_follows_naomit_mother
  )
vif.mer(glm_SIN_females_selected)
glm_SIN_females_selected_low <-lmer(trj_sinuosity~ ageOfCurrentOffspring  + DJL+ fruittreecount+meanTimelag+ (1 | monthYearDate) + (1 | focal), # random effects to correct for
  data = SIN_follows_naomit_mother
  )
anova(lmm_SIN_females_empty,glm_SIN_females_selected)
vif.mer(glm_SIN_females_selected_low)
...
summary(glm_SIN_selected_backward)
vif.mer(glm_SIN_selected_backward)
anova(lmm_SIN_females_empty,glm_SIN_selected_backward)
r.squaredGLMM(glm_SIN_selected_withfruittree)
...
sjPlot::tab_model(glm_SIN_selected, 
                  ...
                  dv.labels= "Effects of Herbivores on Coral Cover")
?tab_model
summary(glm_SIN_females_selected)
sjPlot::tab_model(glm_SIN_females_selected,  digits = 4,
                  show.re.var= TRUE, 
                  pred.labels =c("intercept","day rain","waterlevel evening", "night rain","minimum °C evening","minimum °C morning","maximum °C evening","maximum °C morning","average temperature", "FAI", "Class: flanged male", "Class: infant","Class: juvenile","Class: unflanged male","mean timelag","age of offspring","age"),
                  dv.labels= "Effects of internal and external factors on DJL")
ranova(glm_SIN_females_selected)
view(as.data.frame(vif.mer(glm_SIN_females_selected)))
r.squaredGLMM(glm_SIN_females_selected)
anova(lmm_SIN_females_empty,glm_SIN_females_selected)
...
glm_FTC_selected <-lmer(fruittreecount~ rain_eve  + rain_mo +  avg_temp_tot   + fai + ClassFocal  + RR +trj_sinuosity+  (1 | monthYearDate) + (1 | focal), # random effects to correct for
  data = FTC_follows_naomit_full
  )
...
glm_FTC_females_selected <-lmer(fruittreecount~  rain_eve  + rain_mo +  avg_temp_tot   + fai+ dominance + ageOfCurrentOffspring  + RR +trj_sinuosity+  (1 | monthYearDate) + (1 | focal), # random effects to correct for
  data = FTC_follows_naomit_mother
  )
...
summary(glm_FTC_females_selected)
sjPlot::tab_model(glm_FTC_females_selected,  digits = 4,
                  show.re.var= TRUE, 
                  pred.labels =c("intercept","day rain","waterlevel evening", "night rain","minimum °C evening","minimum °C morning","maximum °C evening","maximum °C morning","average temperature", "FAI", "Class: flanged male", "Class: infant","Class: juvenile","Class: unflanged male","mean timelag","age of offspring","age"),
                  dv.labels= "Effects of internal and external factors on DJL")
ranova(glm_FTC_females_selected)
view(as.data.frame(vif.mer(glm_FTC_females_selected)))
r.squaredGLMM(glm_FTC_females_selected)
anova(lmm_FTC_monthYearDate,glm_FTC_selected)
