...
SUAQ_rangepoints_11all20_loc <- SUAQ_rangepoints_11all20_loc %>% mutate(manualTime=if_else(is.na(manualTime),automaticTime,manualTime))
SUAQ_rangepoints_11all20_loc <- SUAQ_rangepoints_11all20_loc %>% mutate(manualDate=if_else(is.na(manualDate),date(automaticDate),manualDate))
SUAQ_rangepoints_11all20_loc <- SUAQ_rangepoints_11all20_loc %>% mutate(manualDatetime=if_else(is.na(manualDatetime),ymd_hms(paste(manualDate,manualTime),tz = "Asia/Pontianak"),manualDatetime))
...
colnames(DJL_follows) <- c("follow","focal","ClassFocal","rain_eve","watlevel_mo","walevel_eve","rain_mo","avg_temp_tot","max_temp_average","min_temp_average","temp_max_mo","temp_min_mo","fai","ageOfCurrentOffspring","monthYearDate","DJL","count","turningAngleSum")
