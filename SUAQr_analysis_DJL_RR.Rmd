...
plot_Allscatter <- DJL_follows %>% ungroup() %>% dplyr::select(follow, ageFromDOB, ClassFocal, rain_eve, watlevel_mo, walevel_eve, rain_mo, avg_temp_tot, max_temp_average, min_temp_average, temp_max_mo,temp_min_mo, fai, ageOfCurrentOffspring, DJL, RR)
...
DJL_follows_naomit_mother <- DJL_follows %>% ungroup() %>%  dplyr::select(DJL,ageFromDOB,rain_eve, walevel_eve, rain_mo, temp_min_eve, temp_min_mo, avg_temp_tot, ClassFocal, fai, ageOfCurrentOffspring, monthYearDate,focal,meanTimelag,numOfPoints) %>% na.omit() # That should be only mothers
...
glm_selected <- lmer(DJL~rain_eve + walevel_eve + rain_mo + temp_min_eve + temp_min_mo + avg_temp_tot + ClassFocal + fai + ageOfCurrentOffspring + ageFromDOB +(1 | monthYearDate)+ (1 | meanTimelag),data=DJL_follows_naomit_mother)
glm_selected_ageInteraction <- lmer(DJL~rain_eve + walevel_eve + rain_mo + temp_min_eve + temp_min_mo + avg_temp_tot + ClassFocal*ageFromDOB + fai + ageOfCurrentOffspring +(1 | monthYearDate)+ (1 | meanTimelag),data=DJL_follows_naomit_mother)
glm_randomEffects <- lmer(DJL~(1 | monthYearDate)+ (1 | meanTimelag),data=DJL_follows_naomit_mother)
anova(glm_selected,glm_selected_ageInteraction,glm_randomEffects)
anova(glm_selected)


### Full model mit NA's
glm_full <-lmer(DJL ~ rain_eve + walevel_eve + rain_mo + temp_min_eve + temp_min_mo + avg_temp_tot + ClassFocal + fai + ageOfCurrentOffspring + (1 | monthYearDate)
