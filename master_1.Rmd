...
bind_rows(suaq_gps_17_18,waypoints) # Missing information about age class in SUAQ_data


# CRS Code: EPSG 32647
# Create SF Points
suaq_orangutan_follows_sf <-  st_as_sf(suaq_gps_17_18, 
                          coords = c("Y", "X"), 
                          crs = 4326)
...
for (n in 1:length(levels(as.factor(SUAQ_data$fileN)))){
  print(paste("loading track number:",2))
...
plot1<- suaq_orangutan_follows_sf %>% 
  group_by(Follow..) %>%
  ggplot(aes(E,N))+
  geom_point(aes(color=suaq_orangutan_follows_sf$Focal),size=0.3)+
  geom_path(aes(color=suaq_orangutan_follows_sf$Focal), alpha = 0.2,size=0.3)
ggplotly(plot1)
