...
#Task: --> change every pttype variable above to a different attribute. See which are categorized in multiple groups!
    ...
    PtType = ifelse((sym == "Circle, Blue" |
                   sym == "Navaid, Blue"),
                    ...
                    "party",
                    PtType),
    "PtType_partner" = ifelse((sym == "Circle, Blue" |
                   sym == "Navaid, Blue")&
...
, collapse="|"), name),
  ...
  PtType_partner_1=unlist(PtType_partner)[1],
  PtType_partner_2=unlist(PtType_partner)[2],
  PtType_partner_3=unlist(PtType_partner)[3]
) %>% 
  select(-PtType_partner)

# "Food Type / Individual / Direction"
...
do <- flattenList(tmp)
mydf = data.frame(x1 = c(1:length(tmp)))
mydf$x2 = tmp
mydf$x2[[1]][1]
mydf<- mydf %>% mutate(index = row_number()) %>% 
  mutate(orangutans_test=ifelse(lengths(.$x2)==0,NA,.$x2[index])) %>% 
  mutate(orangutans_indi=ifelse(is.na(.$orangutans_test),NA,.$orangutans_test[][[1]]))
test<- unlist(mydf$orangutans_test[1])
first(test)
mydf %>% mutate(test=first(.$orangutans_test))
mydf<- mydf %>% rowwise() %>%  mutate(indidi=get_vector_element(.$x2,row_number()))
get_vector_element<- function(x,y){
  x[y]
}
