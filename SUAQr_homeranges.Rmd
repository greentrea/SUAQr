...
for(per in amountofdata){
  ...
  for (i in selected_females) { # do the following for every animal in the list
      ...
      selectedfollows <- SUAQ_rangepoints_11all20_loc_females_10 %>% filter(focal%in%c(i)) %>% sample_n(round(nrow(follows)*per)) #use when  subsetting by follow not but by number of GPS
      if(nrow(selectedfollows)>0){
...
kde95_bootstrap <- data.frame(focal=as_vector(selected_females),
                 HPI_area_5=vector(mode="numeric", length=10),             
                 HPI_area_10=vector(mode="numeric", length=10),
                 HPI_area_15=vector(mode="numeric", length=10),
                 HPI_area_20=vector(mode="numeric", length=10),
                 HPI_area_25=vector(mode="numeric", length=10),
                 HPI_area_30=vector(mode="numeric", length=10),
                 HPI_area_35=vector(mode="numeric", length=10),
                 HPI_area_40=vector(mode="numeric", length=10),
                 HPI_area_45=vector(mode="numeric", length=10),
                 HPI_area_50=vector(mode="numeric", length=10),
                 HPI_area_55=vector(mode="numeric", length=10),
                 HPI_area_60=vector(mode="numeric", length=10),
                 HPI_area_65=vector(mode="numeric", length=10),
                 HPI_area_70=vector(mode="numeric", length=10),
                 HPI_area_75=vector(mode="numeric", length=10),
                 HPI_area_80=vector(mode="numeric", length=10),
                 HPI_area_85=vector(mode="numeric", length=10),
                 HPI_area_90=vector(mode="numeric", length=10),
                 HPI_area_95=vector(mode="numeric", length=10),
                 HPI_area_100=vector(mode="numeric", length=10),
...
for (i in selected_females){ # compute KDE home range for every individual
  ...
  follows <- SUAQ_rangepoints_11all20_loc_females_10 %>% filter(focal%in%c(i)) %>% group_by(follow) %>% summarise()
  selectedfollows <- SUAQ_rangepoints_11all20_loc_females_10 %>% filter(focal%in%c(i)) %>% sample_n(round(nrow(follows)*per))
  xy_selected_perc <- SUAQ_rangepoints_11all20_loc_females_10 %>% filter(follow %in% pull(selectedfollows,follow))# use when  subsetting by follow not but by number of GPS
  #xy_selected <- SUAQ_rangepoints_11all20_loc_females_10 %>% filter(focal%in%c(i)) # use when not subsetting by follow but by number of GPS
  ...
  for (j in seq(1, length(methList))) { # compute one home range for each bandwidth method and individual
    if (j == 1) {
      UD <- (kernelUD(xy_selected, grid=gridVal, extent=2, h=as.numeric(bandwiths[iterator,"hpi"]))) 
    }
    else {
      print("Error")
    }
...
for(n in as_vector(selected_females)){
  print(n)
  try(kde50_bootstrap[itera,paste("HPI_area","_",as.character(100*per),sep = "")] <-  kde_list[[n]]$HPI$hr50$area,silent = TRUE)
  try(kde95_bootstrap[itera,paste("HPI_area","_",as.character(100*per),sep = "")] <-  kde_list[[n]]$HPI$hr95$area,silent = TRUE)
  itera <- itera+1
}
}
  }
...
kde_bootstrap_monthly <- kde_bootstrap_monthly %>% mutate(algorithm="HPI")
st_write(kde_bootstrap_monthly, dsn = paste(
  paste(imdir,
    "kde_bootstrap_monthly_perfollow_HPI",
    sep = ""
  ),
  "csv",
  sep = "."
...
for(per in amountofdata){
  ...
  selectedfollows<- sample_n(SUAQ_rangepoints_11all20_loc_females_10, round(nrow(follows)*per))
...
for(algo in vecalgo){
  ...
  for (hr in vechomreange){
    ...
    if(algo=="ctmm"){periods<- levels(factor(as.factor(all_homeranges_sf_selected[all_homeranges_sf_selected$algorithm=="ctmm",]$period)))}
...
all_overlaps <- all_overlaps_sf %>% st_drop_geometry() %>% filter(bndwthm %in% c(NA, "HSCV")) %>% filter(period %in% c("total research period")) %>% filter(hminfac %in% c(NA,1))%>% filter(modelbase %in% c(NA,"unpooled"))
...
### Create a non sf object aswell
all_homeranges <- all_homeranges_sf %>% st_drop_geometry()
all_homeranges_selected <- all_homeranges %>% filter(bndwthm %in% c(NA, "HSCV")) %>%filter(hminfac %in% c(NA, "0.9")) %>%filter(modelbase %in% c(NA,"unpooled"))
...
#### Homerange estimators make table with qualitative HRE evaluation
test<- all_homeranges %>% group_by(algorithm,bndwthm,period,hminfac) %>% 
  summarise(area=(mean(area,na.rm=TRUE)/max(all_homeranges$area, na.rm = TRUE)),                                                   holearea=(mean(Comp_holearea,na.rm=TRUE)/max(as.numeric(all_homeranges$Comp_holearea), na.rm = TRUE)),
            Comp_polycount=(mean(Comp_polycount,na.rm=TRUE)/max(all_homeranges$Comp_polycount, na.rm = TRUE)),
            Comp_compactness=(mean(Comp_compactness,na.rm=TRUE)/max(all_homeranges$Comp_compactness, na.rm = TRUE)),
)
...
kde_evaluation<- all_homeranges %>% dplyr::filter(algorithm=="kde") %>% dplyr::filter(period =="total research period")%>%   group_by(focal,hrprcnt,bndwthm) %>% 
  summarise(area=(mean(area,na.rm=TRUE)),holearea=(mean(Comp_holearea,na.rm=TRUE)),
            Comp_polycount=(mean(Comp_polycount,na.rm=TRUE)),
            Comp_compactness=(mean(Comp_compactness,na.rm=TRUE)),
            AUC=(mean(AUC,na.rm=TRUE))
)
kde_evaluation_temp <- kde_evaluation %>% ungroup() %>%  group_by(focal, hrprcnt) %>% summarise(bndwthm,area=area/max(as.numeric(area), na.rm = TRUE),holearea=holearea/max(as.numeric(holearea), na.rm = TRUE),
...
plot_kdeeval<- kde_evaluation %>% ggplot(aes(focal, norm_value,color=as.factor(bndwthm)))+
  ...
  facet_grid(evaltype~hrprcnt,labeller = labeller(evaltype=as_labeller(
     c(norm_AUC="AUC",norm_area="area",norm_compactness="compactness",norm_holes="holes area",norm_polygons="polygons"))),scales="free")+
...
brb_evaluation_temp<- brb_evaluation %>% ungroup() %>%  group_by(focal, hrprcnt) %>% summarise(hminfac,area=area/max(as.numeric(area), na.rm = TRUE),holearea=holearea/max(as.numeric(holearea), na.rm = TRUE),
            Comp_polycount=Comp_polycount/max(as.numeric(Comp_polycount), na.rm = TRUE),
            Comp_compactness=Comp_compactness/max(as.numeric(Comp_compactness), na.rm = TRUE),
)
colnames(brb_evaluation_temp) <- c("focal","hrprcnt","hminfac","norm_area","norm_holes","norm_polygons","norm_compactness")
brb_evaluation <- brb_evaluation %>% 
  left_join(brb_evaluation_temp,by=c("hrprcnt"="hrprcnt","focal"="focal","hminfac"="hminfac")) %>% dplyr::select(focal,hrprcnt,hminfac, norm_area,norm_holes,norm_polygons,norm_compactness)

brb_evaluation<- gather(brb_evaluation,"evaltype","norm_value",4:7)

plot_brbeval<- brb_evaluation %>% ggplot(aes(focal, norm_value,color=as.factor(hminfac)))+
  geom_point(shape= 1,position = position_dodge(width = 0.2),size=2)+
  facet_grid(evaltype~hrprcnt,labeller = labeller(evaltype=as_labeller(
     c(norm_area="area",norm_compactness="compactness",norm_holes="holes area",norm_polygons="polygons"))))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size = 7))+
...
plot_kde_bootstrap_monthly<- kde_bootstrap_monthly %>% 
  ggplot(aes(`subset %`,`HR [km2]`,colour=hrpercentage)) + geom_jitter(alpha=0.2)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size = 7))+
  facet_wrap(.~ focal ,ncol=5,nrow=2)
...
colnames(kde_bootstrap_monthly_perfollow) <- c("focal","percentagedata","homerange","iteration","hrpercentage")
brb_temporal_bootstrap_monthly <- brb_temporal_bootstrap_monthly %>% mutate(algorithm = "BRB hmin factor 0.8")
colnames(brb_temporal_bootstrap_monthly)[colnames(brb_temporal_bootstrap_monthly) == 'homerange'] <- 'area'
kde_bootstrap_monthly_perfollow <- kde_bootstrap_monthly_perfollow %>% mutate(algorithm = "KDE HSCV", percentagedata=percentagedata/100)
mcp_bootstrap_monthly_perfollow <- mcp_bootstrap_monthly_perfollow %>% mutate(algorithm = "MCP",percentagedata=percentagedate)

all_bootstrapped<- brb_temporal_bootstrap_monthly %>% bind_rows(kde_temporal_bootstrap_monthly)%>% bind_rows(mcp_bootstrap_monthly_perfollow)

plot_brb_temporal_bootstrap_monthly<- all_bootstrapped %>% 
  filter(hrpercentage!="99%") %>% 
  filter(hminfac%in%c(0.8,NA)) %>%
  arrange(desc(percentagedata)) %>% 
  ggplot(aes(percentagedata,area,colour=focal)) + geom_path()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size = 10))+
  scale_fill_viridis()+
  #scale_x_discrete(breaks=breaks_vec)+
  #scale_colour_manual(values = c("50%" = "#1b9e77", "95%" = "#d95f02", "99%" = "#7570b3"))+
  scale_size_continuous(range = c(0.6, 7))+
  labs(y=expression("area [km"^2*"]"),x= "percentage of data",color="Orangutan")+
  facet_wrap(hrpercentage~ algorithm,scale="free")
  #geom_path()
...
all_homeranges_sf %>%filter(algorithm=="brb") %>% 
  filter(hminfac==1) %>%  
  ggplot(aes(focal,first_nn_average))+
  geom_point(aes(color=hrprcnt),size=3)+
  geom_smooth()+
  labs(x="focal",y=expression("normalized trees [tree/tot gps effort]"),color="HR [%]") +
  scale_fill_viridis()
...
all_overlaps_sf %>%
  ...
  #filter(algorithm=="brb") %>% 
  filter(hminfac%in%c(1,NA)) %>% # filter only brb with 1 --> reduce redundancy
...
test<- all_overlaps_sf %>%
  ...
  filter(bndwthm%in%c("HBCV",NA)) %>% # filter only kde with hbcv method
  filter(modelbase%in%c("pooled",NA)) %>%  # filter only ctmm with pooled method
  filter(period=="2012 until 2016") %>% 
  filter(focal=="cissy")


### Testing make overlap dynamics visible
all_overlaps_50_1HBCVPOOLED <- all_overlaps_sf %>%
  mutate(period = fct_relevel(period, 
            "before 2012", "2012 until 2016", "2016 until August 2018", 
            "Since August 2018", "total research period")) %>%
  #filter(algorithm=="brb") %>% 
  ...
  filter(algorithm=="brb") %>% 
  filter(hrprcnt=="50%") %>% 
  dplyr::select(focal,overlap,focal.1,period)%>% 
  #mutate(focal.1=paste0("overlapping "," ",focal.1)) %>% 
  st_drop_geometry() %>% 
  spread(focal.1,overlap) %>% 
  filter(period=="2012 until 2016") %>% dplyr::select(-period)
all_overlaps_50_1HBCVPOOLED[is.na(all_overlaps_50_1HBCVPOOLED)] <- 0

rownames(all_overlaps_50_1HBCVPOOLED) <- all_overlaps_50_1HBCVPOOLED$focal

all_overlaps_50_1HBCVPOOLED <- all_overlaps_50_1HBCVPOOLED %>% dplyr::select(-focal)
# Load the circlize library
library(circlize)
 
# Make the circular plot

grid.col <- c(cissy = "#006d2c", ellie = "#045a8d", friska = "#3690c0",
    lilly = "#41ae76", lisa = "#99d8c9", raffi = "#525252", sarabi = "#e5f5f9", tiara = "#969696", trident = "#d9d9d9",yulia="#a6bddb")

SUAQ_rangepoints_11all20_loc_females_10 %>% group_by(focal,SUAQ_orangutans.matriline) %>% summarise() %>% view()

sankeyNetwork(all_overlaps_50_1HBCVPOOLED, transparency = 0.25,
  grid.col = c("red", "blue", "purple","pink", "orange", "green", "steelblue1"),)

sankeyNetwork(Links = all_overlaps_50_1HBCVPOOLED, Nodes = nodes,
...
## MATRILINE
all_homeranges_50_naomit <- all_homeranges_50 %>% dplyr::select(area,matriline,period,focal,n_gps,n_fllws,algorithm,fruittree_normalized) %>%  na.omit()
all_homeranges_95_naomit <- all_homeranges_95 %>% dplyr::select(area,matriline,period,focal,n_gps,n_fllws,algorithm,fruittree_normalized) %>% na.omit()
...
all_overlaps_95_naomit <- all_overlaps_95 %>% dplyr::select(area,n_gps,overlap,overlap_jaccard,related,period,focal,focal.1,algorithm) %>% na.omit() %>% ungroup()
...
#### Core area
lmer_jaccard_50 <- lmer(overlap_jaccard ~ related+(1 | focal.1)+(1 | focal)+(1| n_gps)+(1| algorithm),all_overlaps_50_naomit,REML=FALSE)
...
#### 95% Area
lmer_numOfPoints95 <- lmer(overlap_jaccard ~ related+(1 | focal.1)+(1 | focal)+(1| n_gps)+(1| algorithm),all_overlaps_95_naomit,REML=FALSE)
...
all_overlaps_50_naomit <- all_overlaps_50%>%  dplyr::select(area,n_gps,overlap,fruittree_normalized,related,period,focal,focal.1,algorithm) %>% na.omit() %>% ungroup() 
all_overlaps_95_naomit <- all_overlaps_95 %>% dplyr::select(area,n_gps,overlap,fruittree_normalized,related,period,focal,focal.1,algorithm) %>% na.omit() %>% ungroup()
lmer_overlap_all_nullmodell_95 <- lmer(fruittree_normalized ~ (1 | focal)+(1| n_gps)+(1|algorithm),all_overlaps_95_naomit,REML=FALSE)
lmer_overlap_all_nullmodell_50 <- lmer(fruittree_normalized ~ (1 | focal)+(1| n_gps)+(1| algorithm),all_overlaps_50_naomit,REML=FALSE)

all_overlaps_95_naomit %>% ggplot(aes(as.factor(related),fruittree_normalized,colour=area))+
  geom_boxplot() +
  geom_jitter()
...
### Fruittreeindex
temp_fruittreediffoverlaps<- all_overlaps %>% dplyr::select(area=overlap,fruittree_normalized,algorithm) %>% mutate(type="overlap") 
temp_fruittreediffoverlaps2<- all_homeranges_sf %>% dplyr::select(area,fruittree_normalized,algorithm)%>% mutate(type="homerange")%>% st_drop_geometry()
temp_fruittreediffoverlaps <- temp_fruittreediffoverlaps %>% rbind(temp_fruittreediffoverlaps2)

temp_fruittreediffoverlaps %>% ggplot(aes(as.factor(type),fruittree_normalized,colour=area))+
  geom_boxplot() +
  geom_jitter()
