    ...
    tryCatch(
      ...
      tryCatch(
            ...
            error = function(e){ 
              ...
              hr99_sf <- NA
              message()
              message(paste(selected_females[[iterator]],hminFactor[[l]],n_gps,n_follows,samplinginterval,sep = " : "))
            }
        )
      tryCatch(
            expr = {
            ...
            error = function(e){ 
              hr50 <- NA
              hr50_sf <- NA
              message(paste(selected_females[[iterator]],hminFactor[[l]],n_gps,n_follows,samplinginterval," getting hr percentage polygon didn't work",sep = " : "))
            }
        )
    
    ########## save UD ##########
    kde_overlap_UD_period[[1+indexitter]] <- list(selected_females[[iterator]],UD,"kde",methList[[j]],n_gps,n_follows,samplinginterval)
  ...
  for (hr in vechomreange){
    ...
    for (peri in periods){
      ...
      for(overlap in c(1:NROW(focalcombination))){
        ...
        ### Testing KDE
        if(algo=="kde"){
        boolean<- kde_overlap_UD %>% map(~ has_element(.x, c(as.character(focalcombination[overlap,1])))) %>% as_vector() # get the right algorithm
        focal1_UD<- kde_overlap_UD[boolean]
        ...
        boolean<- kde_overlap_UD %>% map(~ has_element(.x, c(as.character(focalcombination[overlap,2])))) %>% as_vector() # get the right algorithm
        focal2_UD<- kde_overlap_UD[boolean]
        ...
        hr95_1_sf<- st_as_sf(hr95_1) %>% st_set_crs(23847)
        hr95_2_sf <- st_as_sf(hr95_2)%>% st_set_crs(23847)
        ggplot(data=hr95_1_sf_crs)+geom_sf(color="green",fill=NA)+geom_sf(data=hr95_1_sf_stamp,color="blue")+geom_sf(data=hr95_2_sf,color="red",fill=NA)
        
        
        
        
        currentUDoverlap_PHR<- kerneloverlaphr(estUDm_pseudo, meth="PHR", conditional=TRUE)
        currentUDoverlap_HR<- kerneloverlaphr(estUDm_pseudo, meth="HR", conditional=TRUE)
        currentUDoverlap_VI<- kerneloverlaphr(estUDm_pseudo, meth="VI", conditional=TRUE)
                print(hellodsf)
        }
        ###
...
adehabitatHR
result<- kerneloverlaphr(test, meth="HR", percent = 95)

hr95_1 <- getverticeshr(UD1, percent=50,unin = c("m"),unout = c("km2"))
hr95_2 <- getverticeshr(UD2, percent=50,unin = c("m"),unout = c("km2"))
hr95_1_sf <- st_as_sf(hr95_1,crs=) %>% st_set_crs(CRS( "+proj=utm +zone=47 +a=6378160 +b=6356774.50408554 +towgs84=-24,-15,5,0,0,0,0 +units=m +no_defs" ))
hr95_2_sf <- st_as_sf(hr95_2)%>% st_set_crs(CRS( "+proj=utm +zone=47 +a=6378160 +b=6356774.50408554 +towgs84=-24,-15,5,0,0,0,0 +units=m +no_defs" ))
ggplot(data=hr95_1_sf)+geom_sf(fill=NA)+geom_sf(data=hr95_2_sf,color="red",fill=NA)

test<- list(focal1=UD1,focal2=UD2, focal3=UD1)
image(test)
class(test) <- "estUDm"
test<- kde_overlap_UD[[1]][[2]]
test2<- kde_overlap_UD[[2]][[2]]
...
all_overlaps_sf<- st_read(dsn="/Users/stefgr/Nextcloud/UZH/12_Semester/20200313_Masterthesis/R/data/Processed_data/all_overlaps_jaccard.shp")
...
all_overlaps <- all_overlaps_sf %>% st_drop_geometry()
...
### Finding: In core areas the number of trees increase linear with area in 95% area it increases faster but then flattens
all_overlaps  %>%  ggplot(aes(jaccard_overlap,number_of_trees,color=algorithm)) +geom_point()
all_homeranges_sf %>% ggplot(aes(area,fruittree_normalized,color=hrprcnt)) + geom_point()
