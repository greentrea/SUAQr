...
packages_checker(
  c('hrbrthemes', # for colors --> viridis
    'rgdal',
    'ctmm',
    'lubridate',
    'plyr',
...
SUAQ_waypoints_11all20_loc <-
  read_csv(paste0("data/Processed_data/",str_remove_all(Sys.Date()-1, "-"),"_SUAQ_waypoints_11all20.csv"),guess_max = min(10000, 30000))
...
dir.create(paste(
    "output/Homeranges/KDE_Periods/researchperiods/",
    format(Sys.Date(), "%Y%m%d"),
    sep = ""
  ))
...
```{r BRB - HR sizes,echo=FALSE, message=FALSE,comment=FALSE, warning=FALSE}
# Split the data into 4 sampling periods. Calculate the respective HR.
SUAQ_rangepoints_11all20_loc_females_10 <- SUAQ_rangepoints_11all20_loc_females_10 %>% mutate(year=year(manualDatetime))
...
for (samplinginterval in c(1:length(mt_list))){
  ...
  for (i in selected_females) { # do the following for every animal in the list
    ...
    # only select points where timegaps are between 20-40min
...
brb_list %>%filter(hminfac==1) %>%   ggplot(aes(period,hr95,colour=focal)) + geom_point(size=0.3)+geom_path(data=brb_list,aes(fill=period),fill=period)

brb_list_reshape <- brb_list %>% filter(hminfac%in%c(1)) %>%  dplyr::select(focal,hr50,hr95,period) %>% na.omit()
brb_list_reshape <-  spread(brb_list_reshape,period,hr95) %>% dplyr::select(-hr50)
brb_list_reshape<- setDT(brb_list_reshape)[, lapply(.SD, function(x) unique(na.omit(x))), by = focal] %>% dplyr::select(focal,`before 2012`,`2012 until 2016`,`2016 until August 2018`,`Since August 2018`,`total research period`) %>% ungroup()
str(brb_list_reshape)
colnames(brb_list_reshape) <- c("focal","R1","R2","R3","R4","R5")
#brb_list_reshape[is.na(brb_list_reshape)] <- 0
brb_list_reshape <- brb_list_reshape %>% rowwise() %>% mutate(mean=mean(c(R1,R2,R3,R4,R5),na.rm=TRUE))
brb_list_reshape <- brb_list_reshape %>% mutate(R1=if_else(is.na(R1),mean,R1))
brb_list_reshape <- brb_list_reshape %>% mutate(R2=if_else(is.na(R2),mean,R2))
brb_list_reshape <- brb_list_reshape %>% mutate(R3=if_else(is.na(R3),mean,R3))
brb_list_reshape <- brb_list_reshape %>% mutate(R4=if_else(is.na(R4),mean,R4))
brb_list_reshape <- brb_list_reshape %>% mutate(R5=if_else(is.na(R5),mean,R5))


brb_list_reshape <- brb_list %>% na.omit()
plot(brb_list$hr95~brb_list$period , type="b" , bty="l" , xlab="value of a" , ylab="value of b" , col=rgb(0.2,0.4,0.1,0.7) , lwd=3 , pch=17 , ylim=c(1,5) )

ggparcoord(brb_list_reshape,
    columns = 2:6, groupColumn = 1, order = "anyClass",
    showPoints = TRUE, 
    title = "Homerange sizes: BRB - periods",
    alphaLines = 1
    ) + 
