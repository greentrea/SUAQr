...
SUAQ_waypoints_11all20_loc <- read_csv(paste0("data/Processed_data/",str_remove_all(Sys.Date()-1, "-"),"_SUAQ_waypoints_11all20.csv"))
...
```
### Analysis
```{r MCP - HR sizes  Loader,echo=FALSE, message=FALSE,comment=FALSE, warning=FALSE}
...
```{r KDE - HR sizes  Loader,echo=FALSE, message=FALSE,comment=FALSE, warning=FALSE}
...
bandwiths <- df <- data.frame(focal=as_vector(selected_females),
...
for (i in selected_females){ # compute KDE home range for every individual
  ...
  xy_selected_sp <- SpatialPoints(xy_selected,proj4string = CRS( "+proj=utm +zone=47 +datum=WGS84 +units=m +no_defs" )) # Converts SpatialPointDataFrame to SpatialPoint (only x & y)
  ...
  for (j in seq(1, length(methList))) { # compute one home range for each bandwidth method and individual
    ...
    hr95 <- getverticeshr(UD, percent=95)
    hr50 <- getverticeshr(UD, percent=50)
    
    ########## plot results ##########
    imdir <- paste("~/Users/stefgr/Dropbox/Apps/Overleaf/Msc_Thesis/figures/Homeranges/KDE/plots/", "/", sep="")
    imdirUD <- paste("~/Users/stefgr/Dropbox/Apps/Overleaf/Msc_Thesis/figures/Homeranges/KDE/plots/UD/", "/", sep="")
    
    png(paste(imdirUD, selected_females[i], "_KDE_UD_", methList[[j]], ".png", sep=""), width=900, height=700, res=120, units="px")
    image(UD, xlab="x [m]")
    par(mar=c(5.1,1,4.1,1))
    step <- 20000
    xmin <- min(UD@coords[,1]); xmax <- max(UD@coords[,1])
    ymin <- min(UD@coords[,2]); ymax <- max(UD@coords[,2])
    yaxpos <- step*round(xmin/step)-step/5
    plot(hr50, lty=4, lwd=3, border="black", add=T,axes=F)
    plot(hr95, lty=1, lwd=2, border="blue", add=T, axes=F)
    axis(1, at=c(step*(ceiling(xmin/step)):(round(xmax/step))))
    axis(2, at=c(step*(round(ymin/step)):(round(ymax/step))), pos=xmin-5000)
    text(xmin-5000, ymin+(ymax-ymin)/2, "y [m]", adj=c(NA,-4), srt=90)
    title(paste(selected_females[i], ": KDE ", "(", methListTitle[[j]], ")", sep=""), line=-0.3)
    legend("topright", c("HR 50%", "HR 95%"), col=c("black", "blue"), lwd=c(3,2), lty=c(4,1), inset=c(0.14,0.06))
    dev.off()
    
    png(paste(imdir, selected_females[i], "_KDE_VUD_", methList[[j]], ".png", sep=""), width=900, height=700, res=120, units="px")
    image(getvolumeUD(UD), xlab="x [m]")
    par(mar=c(5.1,1,4.1,1))
    step <- 20000
    xmin <- min(UD@coords[,1]); xmax <- max(UD@coords[,1])
    ymin <- min(UD@coords[,2]); ymax <- max(UD@coords[,2])
  ...
  assign(selected_females[i], animRes) # is renamed according to the ith element of animList
  print(paste(selected_females[i], ": Calculation finished", sep=""))
  
  iterator <- iterator+1
}

kde <- list(PF07_Ronja = PF07_Ronja, PM07_Mothamongwe = PM07_Mothamongwe, PM08_Bogarigka = PM08_Bogarigka, PM09_Gham = PM09_Gham,
            LF12_Verity = LF12_Verity, LF13_Ella = LF13_Ella, LF16_Jane = LF16_Jane, LM06_Hitchcock = LM06_Hitchcock,
            LM07_Mexico = LM07_Mexico, LF14_Madge = LF14_Madge, LM08_Orange = LM08_Orange, LF07_Getika = LF07_Getika)   
    
save(kde, file = "~/_MyData/Masterarbeit Movement Analysis/Resultate/thesis/calculations/KDE/KDE.RData")    
