...
xmin <- min(SUAQ_rangepoints_11all20_loc_females_10$E)-500; xmax <- max(SUAQ_rangepoints_11all20_loc_females_10$E)+500
ymin <- min(SUAQ_rangepoints_11all20_loc_females_10$N)-500; ymax <- max(SUAQ_rangepoints_11all20_loc_females_10$N)+500
step <- 200
...
for (samplinginterval in c(1:length(mt_list))){
  ...
  for (i in selected_females) { # do the following for every animal in the list
    ...
    proj4string(xy_selected) <- CRS( "+proj=utm +zone=47 +datum=WGS84 +units=m +no_defs" )
    ...
    iterator <- iterator+1
      ...
      title(paste(i, ": MCP", sep=""), line=-1.5)
      legend("topright", c("HR 50%", "HR 95%", "HR 100%","Trail"), col=c("coral1", "cadetblue2", "darkturquoise","darkolivegreen"), lwd=c(2.5,2,2,0.3), lty=c(3,1,4,1), inset=c(0,0))
      dev.off()
      }
...
for (i in selected_females){ # compute KDE home range for every individual
  ...
  for (j in seq(1, length(methList))) { # compute one home range for each bandwidth method and individual
    ...
    legend("topright", c("HR 50%", "HR 95%", "HR 99%","Trail"), col=c("darkseagreen3", "cadetblue2", "darkturquoise","darkolivegreen"), lwd=c(1.5,1,1,0.3), lty=c(3,1,4,1), inset=c(0,0))
    dev.off()
    ...
    title(paste(selected_females[[iterator]], ": KDE ", "(", methListTitle[[j]], ")", sep=""), line=-1.5)
    legend("topright", c("HR 50%", "HR 95%", "HR 99%","Trail"), col=c("azure4", "cadetblue2", "darkturquoise","darkolivegreen"), lwd=c(1.5,1,1,0.3), lty=c(3,1,4,1), inset=c(0,0))
    dev.off()
...
st_write(kde_results_peryear, dsn = paste(
  paste(
    "output/Homeranges/KDE/years/",
    format(Sys.Date(), "%Y%m%d"),"/",
    "kde_values_years",
