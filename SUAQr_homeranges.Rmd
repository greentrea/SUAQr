...
# Load results from homerange creation and results of trees in homeranges
all_homeranges_sf<- st_read(dsn="/Users/stefgr/Nextcloud/UZH/12_Semester/20200313_Masterthesis/R/data/Processed_data/all_homeranges_sf.shp")
all_homeranges_sf <- all_homeranges_sf%>%  mutate(algrthm=ifelse(algrthm=="ctmm","akde",algrthm))
colnames<- read.csv("/Users/stefgr/Nextcloud/UZH/12_Semester/20200313_Masterthesis/R/data/Processed_data/all_homeranges_sf_colnames.csv")
colnames<- as.vector(colnames$x[!colnames$x %in% c("geometry")])
colnames <- c(colnames,c("geometry"))
colnames(all_homeranges_sf) <- colnames
...
temp_matriline<- SUAQ_orangutans %>% group_by(focal=SUAQ_orangutans.orangutanname,matriline=SUAQ_orangutans.matriline) %>% summarise() %>% dplyr::select(focal,matriline) %>% 
...
### Fruittreeindex
fruittrees_brb_08<- all_homeranges_sf %>%filter(algorithm=="brb") %>% 
  filter(hminfac==0.8) %>%  
...
summary(as.numeric(all_homeranges_50[all_homeranges_50$period=="total research period",]$area))
...
all_homeranges_50_naomit <- all_homeranges_50 %>% left_join(overview_djl,by=c("focal"="focal")) %>% 
  left_join(overview_SIN,by=c("focal"="focal")) %>% 
  left_join(overview_RR,by=c("focal"="focal"))

all_homeranges_50_naomit <-dplyr::select(area,matriline,agecat,dominance,period,focal,n_gps,n_fllws,algorithm) %>%  na.omit()
all_homeranges_95_naomit <- all_homeranges_95 %>% dplyr::select(area,matriline,agecat,dominance,matriline,period,focal,n_gps,n_fllws,algorithm) %>% na.omit()
hr_all_nullmodell_95 <- lmer(area ~ (1 | focal) + (1| algorithm),all_homeranges_95_naomit,REML=FALSE)
hr_all_nullmodell_50 <- lmer(area ~ (1 | focal) + (1| algorithm),all_homeranges_50_naomit,REML=FALSE)
...
### Core range
lmm_cr_full <- lmer(area ~ matriline+agecat+n_fllws+(1 | focal) + (1| algorithm),all_homeranges_50_naomit,REML=FALSE)
anova(hr_all_nullmodell_50,lmm_cr_full)
summary(lmm_cr_full)
vif.mer(lmm_cr_full)
```
