...
SUAQ_waypoints_11all20_loc <-
  read_csv(paste0("data/Processed_data/",str_remove_all(Sys.Date(), "-"),"_SUAQ_waypoints_11all20.csv"),guess_max = min(10000, 30000))
...
for (i in selected_females){ # compute KDE home range for every individual
  ...
  for (j in seq(1, length(methList))) { # compute one home range for each bandwidth method and individual
    ...
    ########## plot results ##########
    imdir <- paste("output/Homeranges/KDE", "/", sep="")
    imdirUD <- paste("output/Homeranges/KDE/plots/UD", "/", sep="")
    ...
    axis(2, at=c(step*(round(ymin/step)):(round(ymax/step))), pos=xmin-10)
    text(xmin-10, ymin+(ymax-ymin)/2, "y [m]", adj=c(NA,-4), srt=90)
    title(paste(selected_females[[iterator]], ": KDE ", "(", methListTitle[[j]], ")", sep=""), line=-0.3)
    legend("topright", c("HR 50%", "HR 95%"), col=c("black", "blue"), lwd=c(1,0.6), lty=c(4,1), inset=c(0.14,0.06))
    dev.off()
    ...
    text(xmin-10, ymin+(ymax-ymin)/2, "y [m]", adj=c(NA,-4), srt=90)
    title(paste(selected_females[[iterator]], ": KDE ", "(", methListTitle[[j]], ")", sep=""), line=-0.3)
    legend("topright", c("HR 50%", "HR 95%"), col=c("azure4", "cadetblue2"), lwd=c(1,0.6), lty=c(4,1), inset=c(0.14,0.06))
    dev.off()
...
mt_list = split(SUAQ_rangepoints_11all20_loc_females_10, f = SUAQ_rangepoints_11all20_loc_females_10$cyl)

for (samplinginterval in c(1:4)){}
##### Determine the smoothing parameters
selected_females <- SUAQ_rangepoints_11all20_loc_females_10 %>% group_by(focal) %>% summarise()
selected_females <- as_vector(selected_females)
