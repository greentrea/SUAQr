...
```{r KDE - HR sizes,echo=FALSE, message=FALSE,comment=FALSE, warning=FALSE}
...
```{r KDE - per-researchperiods - HR sizes,echo=FALSE, message=FALSE,comment=FALSE, warning=FALSE}
...
for (samplinginterval in c(1:length(mt_list))){
  ...
  for (i in selected_females) { # do the following for every animal in the list
    ...
    xy_selected <- SUAQ_rangepoints_11all20_loc_females_10 %>% filter(focal%in%c(i)) %>% dplyr::select(E,N,timelag_lead,manualDatetime)
    ...
    for (l in seq(1:length(hminFactor))) {
      ...
      hr95 <- getverticeshr(UD, percent=95, standardize=TRUE)
      hr50 <- getverticeshr(UD, percent=50, standardize=TRUE)
      hr99 <- getverticeshr(UD, percent=99, standardize=TRUE)
      ########## plot results ##########
      ...
      png(paste(imdir,i, "_BRB_UD_", hminTxt, ".png", sep=""), width=1800, height=1400, res=240, units="px")
      image(UD, xlab="x [m]",ylab="y [m]",xlim=c(xmin_xmax_round_steps[1],tail(xmin_xmax_round_steps,n=1)),ylim=c(ymin_ymax_round_steps[1],tail(ymin_ymax_round_steps,n=1))) 
      plot(hr50, lty=3, lwd=1.5, border="darkseagreen3", add=T,axes=F)
      plot(hr95, lty=1, lwd=1, border="cadetblue2", add=T, axes=F)
      plot(hr99, lty=4, lwd=1, border="darkturquoise", add=T, axes=F)
      ...
      png(paste(imdir,i, "_BRB_VUD_", hminTxt, ".png", sep=""), width=1800, height=1400, res=240, units="px")
      image(getvolumeUD(UD), xlab="x [m]",ylab="y [m]",xlim=c(xmin_xmax_round_steps[1],tail(xmin_xmax_round_steps,n=1)),ylim=c(ymin_ymax_round_steps[1],tail(ymin_ymax_round_steps,n=1))) 
      plot(hr50, lty=3, lwd=1.5, border="darkseagreen3", add=T,axes=F)
      plot(hr95, lty=1, lwd=1, border="cadetblue2", add=T, axes=F)
      plot(hr99, lty=4, lwd=1, border="darkturquoise", add=T, axes=F)
      ...
      ########## export results ############
      resultList <- data.frame(focal=i,hr50=hr50$area, hr95=hr95$area,Velocity_percentile99=velocity,GPSuncertainty=relUn,hmin=hmin,tmax=tmax,lmin=lmin,tau=tau,hminfac=hminFactor[[l]],hminRef=hminRef,period=names(mt_list)[samplinginterval],processingtime=duration)
