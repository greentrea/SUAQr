...
all_homeranges_sf <- kde_allperiods_sf %>% bind_rows(brb_allperiods_sf) %>% bind_rows(mcp_allperiods_sf)
all_homeranges_sf$period <- factor(all_homeranges$period, levels = c("before 2012", "2012 until 2016", "left", "2016 until August 2018", "Since August 2018","total research period"))
...
for (i in c(1:NROW(all_homeranges_sf))){
  number_of_pnts_within<- st_intersection(SUAQ_waypoints_11all20_loc_tree_sf_xy$geometry,all_homeranges_sf[i,]) %>% NROW()
...
all_homeranges_sf <- all_homeranges_sf %>% mutate(fruittree_normalized=number_of_trees/researcheffort_ngps) %>% mutate(fruittree_per_sqkm=fruittree_normalized/area)
...
all_homeranges_dense %>% filter(hminfac%in%c(1,NA)) %>% filter(bndwthm%in%c("HREF",NA)) %>%  ggplot(aes(corearea_ratio,focal,color=algorithm)) +geom_point()
...
all_homeranges_sf %>%filter(algorithm=="brb") %>% filter(hminfac==1) %>% filter(hrprcnt=="99%") %>%  ggplot(aes(area,number_of_trees)) +geom_point()+geom_smooth()

### Fruittreeindex
all_homeranges_sf %>%filter(algorithm=="brb") %>% 
  filter(hminfac==1) %>%  
  ggplot(aes(focal,fruittree_per_sqkm))+
  geom_jitter(aes(color=hrprcnt))+
  geom_smooth()+
  labs(x="focal",y=expression("trees per tot. gps-effort per area [normalized tree/km"^2*"]"),color="HR [%]") +
  scale_fill_viridis()
...
#### Homerange sizes overview
kde_allperiods<- st_read(dsn="output/Homeranges/KDE/periods/20210315/kde_values_allperiods.shp") %>% 
  mutate(algorithm="kde") %>% st_drop_geometry()
brb_allperiods<- st_read(dsn="output/Homeranges/BRB/periods/20210315/brb_values_allperiods.shp") %>% 
  mutate(algorithm="brb") %>% rename(area=homerng) %>% st_drop_geometry()
mcp_allperiods<- st_read(dsn="output/Homeranges/MCP/periods/20210316/mcp_values_allperiods.shp") %>% 
  mutate(algorithm="mcp") %>% rename(area=homerng) %>% st_drop_geometry()
kde_allperiods_sf<- st_read(dsn="output/Homeranges/KDE/periods/20210315/kde_values_allperiods.shp") %>% 
  mutate(algorithm="kde") %>% st_set_crs(23847)
brb_allperiods_sf<- st_read(dsn="output/Homeranges/BRB/periods/20210315/brb_values_allperiods.shp") %>% 
  mutate(algorithm="brb") %>% rename(area=homerng) %>% st_set_crs(23847)
mcp_allperiods_sf<- st_read(dsn="output/Homeranges/MCP/periods/20210316/mcp_values_allperiods.shp") %>% 
  mutate(algorithm="mcp") %>% rename(area=homerng) %>% st_set_crs(23847)
all_homeranges_sf <- kde_allperiods_sf %>% bind_rows(brb_allperiods_sf) %>% bind_rows(mcp_allperiods_sf)
all_homeranges <- kde_allperiods %>% bind_rows(brb_allperiods) %>% bind_rows(mcp_allperiods)
all_homeranges$period <- factor(all_homeranges$period, levels = c("before 2012", "2012 until 2016", "left", "2016 until August 2018", "Since August 2018","total research period"))
all_homeranges_sf$period <- factor(all_homeranges$period, levels = c("before 2012", "2012 until 2016", "left", "2016 until August 2018", "Since August 2018","total research period"))


plot_all50_area_perfemale<- all_homeranges %>% filter(hrprcnt=="95%") %>% 
  ggplot( aes(x=focal, y=area, fill=algorithm)) +
    ...
    ggtitle(expression("Core home range areas: KDE"^"95%"*"")) +
...
plot_all50_area_perfemale %>% ggsave(filename = paste(
  paste(
    "/Users/stefgr/Dropbox/Apps/Overleaf/Msc_Thesis/figures/",
    format(Sys.Date(), "%Y%m%d"),"_",
    "plot_95range_BRB_KDE_MCP_area_perfemale",
    sep = ""
  ),
  "png",
  sep = "."
...
brb_allperiods<- st_read(dsn="output/Homeranges/BRB/periods/20210315/brb_values_allperiods.shp") %>% 
  mutate(algorithm="brb") %>% rename(area=homerng) %>% st_drop_geometry()
all_homeranges <- kde_allperiods %>% bind_rows(brb_allperiods)
all_homeranges$period <- factor(all_homeranges$period, levels = c("before 2012", "2012 until 2016", "left", "2016 until August 2018", "Since August 2018","total research period"))

all_homeranges <- all_homeranges %>% filter(period!="total research period")
test <- SUAQ_rangepoints_11all20_loc_females_10[,c("focal","SUAQ_orangutans.matriline", "SUAQ_orangutans.agecategory","SUAQ_orangutans.dominancecat")] %>% group_by(focal,SUAQ_orangutans.matriline,SUAQ_orangutans.agecategory,SUAQ_orangutans.dominancecat) %>% summarise()
all_homeranges_50 <- all_homeranges %>% left_join(test, by=c("focal"="focal")) %>% filter(hrprcnt=="50%")
all_homeranges_95 <- all_homeranges %>% left_join(test, by=c("focal"="focal")) %>% filter(hrprcnt=="95%")

### Matriline
all_homeranges_50_naomit <- all_homeranges_50 %>% dplyr::select(area,SUAQ_orangutans.matriline,period,focal,n_gps,n_fllws,algorithm) %>%  na.omit()
all_homeranges_95_naomit <- all_homeranges_95 %>% dplyr::select(area,SUAQ_orangutans.matriline,period,focal,n_gps,n_fllws,algorithm) %>% na.omit()
...
all_homeranges_50_naomit %>% ggplot(aes(as.factor(SUAQ_orangutans.matriline),area,colour=period))+
  geom_boxplot() +
...
#### core area
lmer_numOfPoints50 <- lmer(area ~ SUAQ_orangutans.matriline+(1 | focal)+(1| n_gps)+(1| algorithm),all_homeranges_50_naomit,REML=FALSE)
...
coefs
#### core area
lmer_numOfPoints95 <- lmer(area ~ SUAQ_orangutans.matriline+(1 | focal)+(1| n_gps)+(1| algorithm),all_homeranges_95_naomit,REML=FALSE)
...
### Age
all_homeranges_50_naomit <- all_homeranges_50 %>% dplyr::select(area,SUAQ_orangutans.agecategory,period,focal,n_gps,n_fllws,algorithm) %>%  na.omit() 
all_homeranges_95_naomit <- all_homeranges_95 %>% dplyr::select(area,SUAQ_orangutans.agecategory,period,focal,n_gps,n_fllws,algorithm) %>% na.omit()
hr_all_nullmodell_95 <- lmer(area ~ (1 | focal)+(1| n_gps)+(1| algorithm),all_homeranges_95_naomit,REML=FALSE)
hr_all_nullmodell_50 <- lmer(area ~ (1 | focal)+(1| n_gps)+(1| algorithm),all_homeranges_50_naomit,REML=FALSE)

all_homeranges_50_naomit %>% ggplot(aes(as.factor(SUAQ_orangutans.agecategory),area,colour=n_gps))+
  geom_boxplot() +
  geom_jitter()
all_homeranges_95_naomit %>% ggplot(aes(as.factor(SUAQ_orangutans.agecategory),area,colour=n_gps))+
  geom_boxplot() +
  geom_jitter()

#### core area
lmer_numOfPoints50 <- lmer(area ~ SUAQ_orangutans.agecategory+(1 | focal)+(1| n_gps)+(1| algorithm),all_homeranges_50_naomit,REML=FALSE)
anova(lmer_numOfPoints50,hr_all_nullmodell_50)
##### Create P values for lm
coefs <- data.frame(coef(summary(lmer_numOfPoints50)))
# use normal distribution to approximate p-value
...
coefs
#### core area
lmer_numOfPoints95 <- lmer(area ~ SUAQ_orangutans.agecategory+(1 | focal)+(1| n_gps)+(1| algorithm),all_homeranges_95_naomit,REML=FALSE)
anova(lmer_numOfPoints95,hr_all_nullmodell_95)
##### Create P values for lm
coefs <- data.frame(coef(summary(lmer_numOfPoints95)))
# use normal distribution to approximate p-value
