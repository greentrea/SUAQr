...
#### Create SF Points
SUAQ_waypoints_11_20 <-
  SUAQ_waypoints_11_20 %>% left_join(SUAQ_orangutans, by = c("orangutanname" =
                                                               "Orangutan name"))
...
SUAQ_waypoints_11_20<- SUAQ_waypoints_17_18 %>%  bind_rows(SUAQ_waypoints_11_20)
...
SUAQ_tracksperorangutanfocal <-
  SUAQ_waypoints_11_20 %>% 
  group_by(fileN, orangutanname, infantname) %>% 
  summarise(count =n()) %>%
  ungroup() %>%
...
SUAQ_tracksperorangutan<- SUAQ_tracksperorangutanfocal %>% left_join(SUAQ_tracksperorangutaninfant,by=c("orangutanname"="infantname")) %>% mutate(NumPointsPerFocal=ifelse(is.na(.$count_infant),.$count_focal,.$count_infant+.$count_focal))
...
family <- family %>%left_join(SUAQ_tracksperorangutan,by=c("name"="orangutanname"))
family <- family %>%left_join(SUAQ_tracksperorangutan[,c(1,4)],by=c("mother"="orangutanname"),na_matches="never")
