...
SUAQ_waypoints_11_20 <-
  ...
  mutate(
    automaticDatetime= if_else(is.na(automaticDatetime),ymd_hms(cmt, tz = "Asia/Pontianak"),automaticDatetime),
...
##### Manual time corrections (quality unsure is it 6 or 7 hours shift)
SUAQ_waypoints_11all20<- SUAQ_waypoints_11all20 %>% mutate(manualTime=if_else(id==24101,automaticTime,manualTime)) # wrong time for one point probably in fn 2047
SUAQ_waypoints_11all20 <-
  SUAQ_waypoints_11all20 %>% 
  mutate(manualDatetime = if_else(follow %in% c(2326,1101277,2341,2336,2331,2409,2412,2415,2343), manualDatetime+hours(7), manualDatetime)) %>%  # wrong time for the whole follow
  mutate(manualTime = if_else(follow %in% c(2326,1101277,2341,2336,2331,2409,2412,2415,2343), hms::as_hms(manualDatetime), manualTime), manualDatetime= ymd_hms(paste(manualDate,manualTime))) 
...
tmp<- tmp%>% filter(count>1) # retrieve duplicates where E N Follow Focal and the ManualDatetime is the same. These are must have attributes.
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% filter(!(id %in% map(tmp$id, 2))) # only select first id's of the duplicates dataframe, and therefore keep the first rows of the duplicates in the dataset. Hopefully they have the most information but this is not sure. To do a function which would select the row with more information.
