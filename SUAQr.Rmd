...
temp1<- SUAQ_waypoints_11all20 %>% filter(automaticDate==as.POSIXct("2020-07-03",format = "%Y-%m-%d",
      tz = "Asia/Pontianak"))
...
#tmpanalysis
temp <- SUAQ_waypoints_11all20 %>% group_by(.$altitude,.$time,.$name,.$sym,.$fileN,.$source,.$filedate,.$infantname,.$followername,.$followtype,.$automaticDatetime,.$manualDatetime,.$manualDate,.$manualTime,.$automaticDate,.$automaticTime,.$PtType,.$E,.$N,.$focal,.$AgeSex,.$Activity) %>% summarise(gpscount=n()) 
temp<- temp %>% filter(as.integer(gpscount)>1)


temp <- SUAQ_waypoints_11all20 %>% group_by(fileN,manualDatetime,automaticDatetime,E,N,focal) %>% summarise(gpscount=n())
temp1 <- SUAQ_waypoints_11all20 %>% group_by(fileN,manualDatetime,automaticDatetime,E,N,focal,follow) %>% summarise(gpscount=n())
temp1<- temp1 %>% filter(as.integer(gpscount)>1)
temp<- temp %>% filter(as.integer(gpscount)>1)

temp <- temp %>% left_join(temp1,by=c("fileN"="fileN","E"="E","N"="N","manualDatetime"="manualDatetime"))

temp2<- temp %>% group_by(automaticDate,focal) %>% summarise(count=n(),sumGPS=sum(gpscount))

#tmpanalysis


tmp<- SUAQ_waypoints_11all20 %>% group_by(name,manualDate,automaticDate,E,N,focal,follow,manualTime,automaticTime) %>% summarise(count=n())
...
### 6.4. Create project Master-Dataframes
