...
SUAQ_waypoints_11all20<- SUAQ_waypoints_11all20 %>% ungroup() %>% 
  ...
  mutate(follow = if_else(is.na(follow),as.numeric(cur_group_id()+1100018),follow))
...
##### Joining info total number of GPS points per follow


##### Focal names in followlog are correct overwrite the ones in GPS data
temp_FN_whereFLogFocalnameIsCorrect<- c("1107","1039","1043","1085","1106","1479","1480","1481","2219","1697","2024","989","990","2192","2193","2167","974","1042","1041","1184","1439","1518","2263","2280","2188","2238","2272","2272","2272","2272","2272","2272","1606","1650","1651","1187","2198","2270","2271","2286","2288","2289")
...
##### Duplicates to delete by id
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% filter(!(id %in% c(8136,8138,8140,8142,8143,8145,8147)))
...
result_totnumfollows <- SUAQ_waypoints_11all20 %>% 
  group_by(follow) %>% summarise(numGPSofFollow=n()) # 1323 Follows
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% left_join(result_totnumfollows,by=c("follow"="follow"))

specs_percentageOfFollowGPSPointsInNight<- SUAQ_waypoints_11all20 %>% filter((manualTime<lubridate::hms("04:00:00"))|(manualTime>lubridate::hms("20:00:00"))) %>% group_by(follow,numGPSofFollow) %>%
  summarise(numberOfGPSinNighresting=n())
specs_percentageOfFollowGPSPointsInNight<- specs_percentageOfFollowGPSPointsInNight %>% mutate(percOfPointsInNight=1/(numGPSofFollow/numberOfGPSinNighresting))




##### Manual time corrections (quality unsure is it 6 or 7 hours shift) there are many wrong especially automatic time zones
SUAQ_waypoints_11all20<- SUAQ_waypoints_11all20 %>% mutate_cond(id==24101,manualTime=automaticTime) # wrong time for one point probably in fn 2047
SUAQ_waypoints_11all20 <-
  SUAQ_waypoints_11all20 %>% 
  mutate_cond(follow %in% c(2326,1101277,2341,2336,2331,2409,2412,2415,2343),manualDatetime = manualDatetime+hours(7)) %>%  # wrong time for the whole follow
  mutate_cond(follow %in% c(2326,1101277,2341,2336,2331,2409,2412,2415,2343),manualTime =  hms::as_hms(manualDatetime), 
...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(PtType=if_else((manualTime==hms::as_hms("09:00:00"))&(follow==1776)&(PtType=="range"),"abnormal off point",PtType))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(PtType=if_else((manualTime%in% c(hms::as_hms("08:00:00"),hms::as_hms("10:00:00")))&(follow==1822),"abnormal off point",PtType))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(PtType=if_else((manualTime%in% c(hms::as_hms("17:30:00")))&(follow==1874),"abnormal off point",PtType))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(manualTime=if_else((automaticTime%in% c(hms::as_hms("16:11:30")))&(follow==1819),hms::as_hms("16:00:00"),manualTime))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(manualTime=if_else((automaticTime%in% c(hms::as_hms("18:02:01")))&(follow==1819),hms::as_hms("18:00:00"),manualTime))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(manualTime=if_else((automaticTime%in% c(hms::as_hms("12:21:30")))&(follow==1819),hms::as_hms("12:21:30"),manualTime))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(manualTime=if_else((automaticTime%in% c(hms::as_hms("17:30:25")))&(follow==671),hms::as_hms("17:30:00"),manualTime))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(manualTime=if_else((automaticTime%in% c(hms::as_hms("06:31:22")))&(follow==1501),hms::as_hms("11:30:00"),manualTime))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(automaticTime=if_else((automaticTime%in% c(hms::as_hms("06:31:22")))&(follow==1501),hms::as_hms("11:31:22"),automaticTime))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(manualTime=if_else((automaticTime%in% c(hms::as_hms("13:09:49")))&(follow==1501),hms::as_hms("18:00:00"),manualTime))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(automaticTime=if_else((automaticTime%in% c(hms::as_hms("13:09:49")))&(follow==1501),hms::as_hms("18:09:49"),automaticTime))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(manualTime=if_else((automaticTime%in% c(hms::as_hms("06:30:58")))&(follow==1761),hms::as_hms("06:30:00"),manualTime))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(PtType=if_else((manualTime%in% c(hms::as_hms("06:30:00"),hms::as_hms("10:00:00"),hms::as_hms("13:00:00")))&(follow==1780),"abnormal off point",PtType))
...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate(manualDatetime=if_else(is.na(manualDatetime),ymd_hms(paste(manualDate,manualTime),tz = "Asia/Pontianak"),manualDatetime))


###### Deleting empty E N or Time
...
id <- rownames(SUAQ_orangutans)
SUAQ_orangutans <- cbind(id=id, SUAQ_orangutans)

names(SUAQ_orangutans) <- paste0("SUAQ_orangutans.", names(SUAQ_orangutans))
SUAQ_waypoints_11all20 <-
  SUAQ_waypoints_11all20 %>% left_join(SUAQ_orangutans[,c(1:10,38:50)], by = c("focal" = "SUAQ_orangutans.orangutanname"))
...
##### Calculate differences of offspring age and focalage
...
SUAQ_waypoints_11all20<- SUAQ_waypoints_11all20 %>%  
  mutate(
    ageOfCurrentOffspring=min(ageOfOffspring1,ageOfOffspring2,ageOfOffspring3,na.rm = TRUE)) 
...
# Adding new id
id <- rownames(SUAQ_waypoints_11all20)
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% dplyr::select(-id)
SUAQ_waypoints_11all20 <- cbind(id=id, SUAQ_waypoints_11all20)

### 6.4. Only keep gps points within Bounding box of 330000-360000N / 323000-328000E because some points are wrong probably the N and the E variable are the same (accidently copied)
...
duplicates <-
  ...
  summarise(
    ...
    id=list(as.numeric(id))
  ) %>% ungroup()
duplicates<- duplicates%>% filter(count>1) # retrieve duplicates where E N Follow Focal and the ManualDatetime is the same. These are must have attributes.
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% filter(!(id %in% map(duplicates$id, 2))) # only select first id's of the duplicates dataframe, and therefore keep the first rows of the duplicates in the dataset. Hopefully they have the most information but this is not sure. To do a function which would select the row with more information.
...
duplicates_datefocal <-duplicates_datefocal %>% unnest_wider(automaticDatetime) %>% 
  ...
  mutate(automaticDatetime1=as.POSIXct(automaticDatetime1,tz = "Asia/Pontianak"),
         ...
         automaticDatetime3=as.POSIXct(automaticDatetime3,tz = "Asia/Pontianak"))
...
duplicates_datefocal <-duplicates_datefocal %>% rowwise() %>% 
  mutate( nearestAutomaticTimeValueKeepTime=
            which.min(abs(c(automaticDatetime1,automaticDatetime2,automaticDatetime3) - manualDatetime))) 
duplicates_datefocal <- duplicates_datefocal %>%   mutate(correctID=id[nearestAutomaticTimeValueKeepTime])
nearestTimeID<- as_vector(duplicates_datefocal$correctID)
allID <- as_vector(duplicates_datefocal$id)
alterID<- setdiff(allID,nearestTimeID) # IDS of points which should be changed because the same animal has the same time already used by a nearer automatic Time point
##### Changeing the manualTime in the main data
SUAQ_waypoints_11all20 <-
  SUAQ_waypoints_11all20 %>% 
  mutate_cond((.$id %in% alterID),manualDatetime=automaticDatetime,manualTime= hms::as_hms(manualDatetime))
...
tmp<- SUAQ_waypoints_11all20[order(SUAQ_waypoints_11all20$manualTime , decreasing = TRUE ),] %>%filter(!is.na(manualDatetime)) %>%  group_by(follow) %>% slice_head()
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% mutate_cond(is.na(manualDatetime),manualDatetime=tmp$automaticDatetime[tmp$follow==follow])
...
#   rowwise()%>%
#   mutate(
#     SUAQ_followlog.ClassFocal_new = if_else((SUAQ_followlog.ClassFocal%in%c("unk")),
#                                         own_valuefinder(
#                                           SUAQ_waypoints_11all20,
#                                           id,
#                                           "id",
...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>