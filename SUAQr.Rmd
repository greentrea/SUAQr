...
ggMarginal(plot_orangutan_followperiods, type="histogram")
...
p2 <- tmp_SUAQ_waypoints_11all20 %>%
  filter(!is.na(focal)) %>%
  filter(SUAQ_orangutans.Sex=="female") %>% 
  ggplot(aes(x=focal ,y=orangutan_num_of_follows,fill = focal)) +
...
grid.arrange(
  arrangeGrob(
    arrangeGrob(blank, p3, ncol = 2, widths = c(5 / 40, 35 / 40)),
    p1,
    heights = c(2 / 8, 6 / 8),
    nrow = 2
  ),
  arrangeGrob(blank, p2, heights = c(3 / 16, 13 / 16), nrow = 2),
  ncol = 2,
  widths = c(5, 1)
)
