...
class(SUAQ_waypoints_17_18$manualTime[1])
strftime(SUAQ_waypoints_11_20_male$manualDatetime[1], format = "%H:%M:%S",tz = "Asia/Pontianak")
?strptime()
...
hms::as_hms(SUAQ_waypoints_11all20$manualTime[1]+dhours(1))
...
SUAQ_waypoints_11all20 <-
  SUAQ_waypoints_11all20 %>% mutate(manualTime = if_else(follow == 2326, hms::as_hms(manualTime +
                                                                                       dhours(7)), manualTime)) # wrong time for the whole follow
SUAQ_waypoints_11all20 <-
  SUAQ_waypoints_11all20 %>% mutate(manualTime = if_else(follow == 1101277, hms::as_hms(manualTime +
                                                                                          dhours(7)), manualTime)) # wrong time for all points
...
specs_followtypeDiffFollowTypeFromlog <- SUAQ_waypoints_11all20 %>% filter(followtype!=SUAQ_followlog.FollowType) %>% group_by(follow,followtype,SUAQ_followlog.FollowType,focal,) %>% summarise()
