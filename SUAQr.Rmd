...
SUAQ_waypoints_11_20_male <-
  SUAQ_waypoints_11_20_male %>% mutate(
    ...
    focal = tolower(.$Focal),
    automaticDatetime = mdy_hm(timestamp, tz = "Asia/Pontianak"),
    automaticDate=date(automaticDatetime),
    automaticTime=hms::as_hms(automaticDatetime)
...
##### Join followlog 
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% 
  left_join(y=tmp,by=c("manualDate"="Date","focal"="FocalName")) # Problem some of them match with multiple points
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  left_join(y=tmp2,by=c("automaticDate"="Date","focal"="FocalName"),keep = FALSE)
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  left_join(y=tmp3,by=c("manualDate"="Date","infantname"="FocalName"),keep = FALSE)
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  left_join(y=tmp4,by=c("manualDate"="Date","followername"="Observer1"),keep = FALSE)
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  left_join(y=tmp5,by=c("manualDate"="Date","followername"="Observer2"),keep = FALSE)
