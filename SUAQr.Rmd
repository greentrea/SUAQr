...
SUAQ_matriline <- SUAQ_matriline %>% rename_all(tolower)
names(SUAQ_matriline) <- paste0("SUAQ_matriline.", names(SUAQ_matriline))

SUAQ_offspring<- read_csv2("data/20201117_offspring.csv")
SUAQ_offspring <- SUAQ_offspring %>% rename_all(tolower)
names(SUAQ_offspring) <- paste0("SUAQ_offspring.", names(SUAQ_offspring))
...
SUAQ_orangutans <- SUAQ_orangutans %>% left_join(SUAQ_matriline, by=c("orangutanname"="SUAQ_matriline.focal"))
SUAQ_orangutans <- SUAQ_orangutans %>% left_join(SUAQ_offspring, by=c("orangutanname"="SUAQ_offspring.focal"))
...
### 0.4 Weather data 
SUAQ_weather<- read_csv2("data/20201117_SUAQ_weather.csv")
SUAQ_offspring <- SUAQ_weather %>% rename_all(tolower)
...
SUAQ_waypoints_17_18 <- SUAQ_waypoints_17_18 %>% 
  mutate(Focal=tolower(Focal), 
         source="Excel data from Caroline 2017 & 2018",
...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  ...
  mutate(follow=if_else((is.na(follow))&!(follow %in% SUAQ_waypoints_11all20$follow),as.numeric(tmp_FollowNumber_M_focal_followname),follow)) %>% # control if FN not already existing somewhere in the dataset because then its probably already given to a follow. But maybe single gps points with missing information about date focal or observer is not matched so hopefully every gps point within the gps follows have the same information and not suddenly some with missing information otherwise they get sorted out here.
...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  mutate(follow=if_else(is.na(follow)&!(follow %in% SUAQ_waypoints_11all20$follow),as.numeric(tmp_FollowNumber_M_focal_followname2),follow)) %>%
...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  mutate(follow=if_else(is.na(follow)&!(follow %in% SUAQ_waypoints_11all20$follow),as.numeric(tmp_FollowNumber_M_focal_concDate),follow)) %>%
...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  mutate(follow=if_else(is.na(follow)&!(follow %in% SUAQ_waypoints_11all20$follow),as.numeric(tmp_FollowNumber_M_focal),follow)) %>%
  mutate(follownr_reliability=if_else((!is.na(follow))&(follownr_reliability=="low: unknown"),"high: date, name",follownr_reliability)) 

SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  mutate(follow=if_else(is.na(follow)&!(follow %in% SUAQ_waypoints_11all20$follow),as.numeric(tmp_FollowNumber_M_focal_automatic),follow)) %>%
  mutate(follownr_reliability=if_else((!is.na(follow))&(follownr_reliability=="low: unknown"),"medium: aDate, name",follownr_reliability)) 

SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  mutate(follow=if_else(is.na(follow)&!(follow %in% SUAQ_waypoints_11all20$follow),tmp_FollowNumber_M_followname,follow)) %>% 
...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  mutate(follow=if_else(is.na(follow)&!(follow %in% SUAQ_waypoints_11all20$follow),tmp_FollowNumber_M_followname2,follow)) %>% 
...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  mutate(follow=if_else(is.na(follow)&!(follow %in% SUAQ_waypoints_11all20$follow),tmp_FollowNumber_M_infant,follow)) %>%
...
temp<- temp %>% filter(as.integer(gpscount)>1) # if this df is not empty --> Look at duplicates they somehow duplicated atfer join.
...
#### Which ones don't have a follownumber. Add a dummy FN.
specs_noFNAfterJoinAndManualAdj <- SUAQ_waypoints_11all20 %>% 
  filter(is.na(follow)) %>% 
  group_by(fileN,focal,.$manualDate,.$automaticDate,.$follow) %>% 
  summarise(count=n()) # new follows without a follownumber

#### Giving a random FN
tmp<- SUAQ_waypoints_11all20 %>% filter(follow>1100018)
tmp<- tmp %>% mutate(follow=NA_real_)
...
# specstest_FocalNameNotMatching <- test_df %>% left_join(SUAQ_orangutans[,c(2,3,6)],by=c("focal"="SUAQ_orangutans.Mother","SUAQ_followlog.FocalName"="SUAQ_orangutans.orangutanname"),keep = FALSE,na_matches = "never",suffix=c("","motherInfant."))
# 
...
### 6.2. Cleaning variable followtype and complete focal name
#SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% left_join()
SUAQ_waypoints_11all20<- SUAQ_waypoints_11all20 %>%  
  mutate(focal=if_else(is.na(focal),SUAQ_followlog.FocalName,focal),
         followtype=if_else(followtype %in%c("Fl"),"FL",follow)) # get missing focal names from SUAQ followlog.
...
tmp_SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% # make a id for every grouped location. Every occuring location fix gets a group ID.
  ...
  mutate(diagnostic_groupedbylocation = group_indices())
...
##### Focal names
temp_FN_whereFLogFocalnameIsCorrect<- c("1039","1043","1085","1106","1479","1480","1481","2219","1697","2024","989","990","2192","2193","2167","974","1042","1041","1184","1439","1518","2263","2280","2188","2238","2272","2272","2272","2272","2272","2272","1606","1650","1651","1187")
...
test<- SUAQ_waypoints_11all20 %>%ungroup() %>%  filter(followtype %in% c("UN","FF"))

levels(as.factor(SUAQ_waypoints_11all20$followtype))
specs_numOfGPSwithFollowtype <- SUAQ_waypoints_11all20 %>%  filter(!is.na(followtype))
...
SUAQ_waypoints_11all20<- SUAQ_waypoints_11all20 %>% 
  ...
  mutate(timelag=as.numeric(difftime(as.POSIXlt(manualTime),
         ...
         speed_next = manual_distToNext/timelag,
         turningAngle=turning_angle(E,N)
         )
SUAQ_waypoints_11all20 %>% ggplot(aes(.$timelag))+geom_histogram(bins = 1000)+scale_y_log10()
SUAQ_waypoints_11all20 %>% ggplot(aes(.$manual_distTolast))+geom_histogram(bins = 1000)+scale_y_log10()
...
plot_distToLats_over_300<-SUAQ_waypoints_11all20 %>% 
  filter((manual_distToNext>100)&( manual_distTolast >100)) %>% 
...
ggplotly(plot_distToLats_over_300)
