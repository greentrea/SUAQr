...
temp1<- SUAQ_waypoints_11all20 %>% filter(.$SUAQ_followlog.ClassFocal=="unk")

SUAQ_waypoints_11all20<- SUAQ_waypoints_11all20 %>% # replace any SexClass value which is unk or NA to the temporally nearest other Sex Class value for this orangutan
  mutate(SUAQ_followlog.ClassFocal=if_else((SUAQ_followlog.ClassFocal=="unk")|is.na(SUAQ_followlog.ClassFocal),
                                           SUAQ_waypoints_11all20$SUAQ_followlog.ClassFocal[which.min(abs(.$manualDate-SUAQ_waypoints_11all20[SUAQ_waypoints_11all20$focal==.$focal,]$manualDate))],
                                           SUAQ_followlog.ClassFocal))

SUAQ_waypoints_11all20<- SUAQ_waypoints_11all20 %>% # replace any SexClass value which is unk or NA to the temporally nearest other Sex Class value for this orangutan
  mutate(SUAQ_followlog.ClassFocal=if_else((SUAQ_followlog.ClassFocal=="unk")|is.na(SUAQ_followlog.ClassFocal),
    own_valuefinder(.,id,"id","unk","SUAQ_followlog.ClassFocal","temporally","manualDate"),
    SUAQ_followlog.ClassFocal))

own_valuefinder(SUAQ_waypoints_11all20,4876,"id","unk","SUAQ_followlog.ClassFocal","temporally","manualDate")

options(max.print=100000)


SUAQ_waypoints_11all20[which(SUAQ_waypoints_11all20$id == 21897),]$manualDate

SUAQ_waypoints_11all20[which(SUAQ_waypoints_11all20$id == 21896),]$manualDate

own_valuefinder <- function(df,pk,replaceterm,pkcol,targetcol,lookuptype,datecol){
    out <- tryCatch(
          ...
          pk <- as.integer(pk)
            ...
            if(missing(datecol)) {
              ...
              if(lookuptype=="temporally"){
                #print("Searching temporally")
                #print(paste(pk,"temp pk"))
                #print(pk)
                date<- df[which(df$id == pk),]$datecol
                #print(paste(df$datecol[pk],"temp date"))
                df_forcorrsponvalue<- df %>% filter(targetcol==df[which(df$id == pk),]$targetcol)
                df_forcorrsponvalue <-  df_forcorrsponvalue %>% mutate(diff_todate=abs(date-datecol))
        ...
        warning=function(cond) {
            ...
            if(targetvalue==replaceterm){message("Didn't find a value")}
            if(targetvalue!=replaceterm){message("Found a value")}
            message("Finished")
        }
    )    
    return(out)
