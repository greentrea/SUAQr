...
SUAQ_waypoints_11_20 <-
  ...
  mutate(
    ...
    manualDate = date(as.POSIXct(ymd(filedate),
                            format = "%Y-%m-%d",
                            tz = "Asia/Pontianak")),
    manualTime = hms::as_hms(str_extract(name, "^\\d{4}")),

  ) %>% 
  mutate(
    automaticDatetime= if_else(is.na(automaticDatetime),ymd_hms(cmt, tz = "Asia/Pontianak"),automaticDatetime),
    manualTime=if_else(is.na(manualTime),ymd_hms(hms::as_hms(manualDatetime),manualTime)),
    manualDate=if_else(is.na(manualDate),date(manualDatetime),manualDatetime)) %>% 
  mutate(
...
tmp6<- SUAQ_followlog[,c(2,4,14)] %>% mutate(tmp_FollowNumber_M_infant=.$FollowNumber) %>%
  dplyr::select(-FollowNumber)
tmp7<- SUAQ_followlog[,c(2,4,19)] %>% mutate(tmp_FollowNumber_M_followname=.$FollowNumber) %>%
  dplyr::select(-FollowNumber)
tmp8<- SUAQ_followlog[,c(2,4,20)] %>% mutate(tmp_FollowNumber_M_followname2=.$FollowNumber) %>%
  dplyr::select(-FollowNumber)
...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  left_join(y=tmp6,by=c("manualDate"="Date","infantname"="FocalName"),keep = FALSE,na_matches = "never")
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  left_join(y=tmp7,by=c("manualDate"="Date","followername"="Observer1"),keep = FALSE,na_matches = "never")
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  left_join(y=tmp8,by=c("manualDate"="Date","followername"="Observer2"),keep = FALSE,na_matches = "never")
...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  ...
  mutate(follownr_reliability=if_else(!is.na(follow),"high: date, name, observer or more","low: unknown")) %>%
  ...
  mutate(follownr_reliability=if_else(!is.na(follow),"high: date, name, observer2","low: unknown")) %>%
  ...
  mutate(follownr_reliability=if_else(!is.na(follow),"high: concDate, name","low: unknown")) %>%
  
  mutate(follow=if_else(is.na(follow),as.numeric(tmp_FollowNumber_M_focal),follow)) %>%
  #dplyr::select(-tmp_FollowNumber_M_focal) %>% 
  mutate(follownr_reliability=if_else(!is.na(follow),"high: date, name","low: unknown")) %>%
  ...
  mutate(follownr_reliability=if_else(!is.na(follow),"medium: aDate, name","low: unknown")) %>%
  
  mutate(follow=if_else(is.na(follow),tmp_FollowNumber_M_followname,follow)) %>% 
  #dplyr::select(-tmp_FollowNumber_M_followname) %>% 
  mutate(follownr_reliability=if_else(!is.na(follow),"low: date, followername=observer1","low: unknown")) %>%
  
  mutate(follow=if_else(is.na(follow),tmp_FollowNumber_M_followname2,follow)) %>% 
  #dplyr::select(-tmp_FollowNumber_M_followname2) %>%
  mutate(follownr_reliability=if_else(!is.na(follow),"low: date, followername=observer2","low: unknown")) %>%

  mutate(follow=if_else(is.na(follow),tmp_FollowNumber_M_infant,follow)) %>%
  #dplyr::select(-tmp_FollowNumber_M_infant) %>% 
  mutate(follownr_reliability=if_else(!is.na(follow),"low: Date, infantname","low: unknown"))
...
specs_noFN <- SUAQ_waypoints_11all20 %>% 
  filter(is.na(follow)) %>% 
  group_by(fileN,focal,.$manualDate,.$automaticDate,.$follow,.$tmp_FollowNumber_M_focal_followname,.$tmp_FollowNumber_M_focal_followname2,.$tmp_FollowNumber_M_focal,.$tmp_FollowNumber_M_focal_concDate,.$tmp_FollowNumber_M_focal_automatic,.$tmp_FollowNumber_M_infant,.$tmp_FollowNumber_M_followname,.$tmp_FollowNumber_M_followname2) %>% 
  summarise(count=n())

test1 <- SUAQ_waypoints_11all20 %>% filter(fileN=="20100115_Ellie_Unk_FL")


test <- SUAQ_followlog %>% filter(.$SUAQ_followlog.Date==as.POSIXct("2011-01-15",
                            format = "%Y-%m-%d",
                            tz = "Asia/Pontianak"))
test <- SUAQ_followlog %>% filter((.$SUAQ_followlog.FocalName=="ellie")&(SUAQ_followlog.GPS=="Yes"))
...
view(as.data.frame(tmp_colnames))
...
SUAQ_waypoints_11all20_test <- mutate_all(SUAQ_waypoints_11all20, .funs=str_to_lower)
SUAQ_waypoints_11all20_test <- SUAQ_waypoints_11all20_test %>%
...
test <- SUAQ_waypoints_11all20 %>% filter(focal!=SUAQ_followlog.FocalName)
test_df<- SUAQ_waypoints_11all20_test %>% filter(focal=="cissy")
test <- SUAQ_waypoints_11all20_test %>% group_by(focal,manualDate,SUAQ_followlog.FocalName,.$follownr_reliability,follow,missmatch_or_FNambiguity) %>% summarise(count=n())
test_df <- test %>% left_join(SUAQ_orangutans[,c(1,4,8)],by=c("focal"="SUAQ_orangutans.orangutanname","SUAQ_followlog.FocalName"="SUAQ_orangutans.Mother"),keep = FALSE,na_matches = "never",suffix=c("","infantMother."))
test_df <- test_df %>% left_join(SUAQ_orangutans[,c(1,4,8)],by=c("focal"="SUAQ_orangutans.Mother","SUAQ_followlog.FocalName"="SUAQ_orangutans.orangutanname"),keep = FALSE,na_matches = "never",suffix=c("","motherInfant."))
colnames(SUAQ_followlog)

test2<- SUAQ_waypoints_11all20 %>% filter(follow==2811)
####################################################################################################################
...
##### Focal names
temp_FN_whereFLogFocalnameIsCorrect<- c("1039","1043","1085","1106","1479","1480","1481","2219","1697","2024","989","990","2192","2193","2167","974","1042","1041","1184","1439","1518","2263","2280","2188","2238","2272","2272","2272","2272","2272","2272","1606","1650","1651")
