...
SUAQ_waypoints_11all20_test <- SUAQ_waypoints_11all20 %>% filter(E>323000 & 
                                                            E<328000 & 
                                                            N>330000 & 
                                                            N<360000)
...
####
write_csv(SUAQ_waypoints_11all20,path="data/backup/SUAQ_waypoints_11_20_backup.csv")
...
SUAQ_waypoints_all.sp<- tmp_SUAQ_waypoints_morethan50gps_11all20[,c(24,25,26)]
coordinates(SUAQ_waypoints_all.sp) <- c("E","N")
proj4string(SUAQ_waypoints_all.sp) <- CRS( "+proj=utm +zone=47 +datum=WGS84 +units=m +no_defs" )

SUAQ_waypoints_all.mcp<- mcp(SUAQ_waypoints_all.sp,percent = 95)

SUAQ_waypoints_all.spgeo <- spTransform(SUAQ_waypoints_all.sp, CRS("+proj=longlat"))
SUAQ_waypoints_all.mcpgeo <- spTransform(SUAQ_waypoints_all.mcp, CRS("+proj=longlat"))

mybasemap <- get_stamenmap(bbox = c(left = min(SUAQ_waypoints_all.spgeo@coords[,1])-0.005, 
                                    bottom = min(SUAQ_waypoints_all.spgeo@coords[,2])-0.005, 
                                    right = max(SUAQ_waypoints_all.spgeo@coords[,1])+0.005, 
                                    top = max(SUAQ_waypoints_all.spgeo@coords[,2])+0.005), 
                           zoom = 12)




mcp(SUAQ_waypoints_all.sp,percent = 95)
graphics.off() 
par("mar") 
par(mar=c(1,1,1,1))
hrs <- mcp.area(SUAQ_waypoints_all.sp, percent = seq(50, 100, by = 5))
