...
##### 
test <- SUAQ_waypoints_11all20 %>% filter(follow==717)
...
SUAQ_waypoints_11all20<- SUAQ_waypoints_11all20 %>% ungroup() %>% 
  group_by(fileN,focal,.$manualDate,.$automaticDate,.$follow,.$tmp_FollowNumber_M_focal_followname,.$tmp_FollowNumber_M_focal_followname2,.$tmp_FollowNumber_M_focal,.$tmp_FollowNumber_M_focal_concDate,.$tmp_FollowNumber_M_focal_automatic,.$tmp_FollowNumber_M_infant,.$tmp_FollowNumber_M_followname,.$tmp_FollowNumber_M_followname2) %>% 
  mutate(follow = if_else(is.na(follow),as.numeric(row_number()+1100018),follow))%>% ungroup()
...
test_df<- tmp_SUAQ_waypoints_11all20 %>% filter(focal=="cissy")
test <- tmp_SUAQ_waypoints_11all20 %>% group_by(focal,manualDate,SUAQ_followlog.FocalName,.$follownr_reliability,follow,missmatch_or_FNambiguity,.$) %>% summarise(count=n())

test_df <- test %>% left_join(SUAQ_orangutans[,c(2,3,6)],by=c("focal"="orangutanname","SUAQ_followlog.FocalName"="Mother"),keep = FALSE,na_matches = "never",suffix=c("","infantMother."))
specstest_FocalNameNotMatching <- test_df %>% left_join(SUAQ_orangutans[,c(2,3,6)],by=c("focal"="Mother","SUAQ_followlog.FocalName"="orangutanname"),keep = FALSE,na_matches = "never",suffix=c("","motherInfant."))
...
test<- SUAQ_waypoints_11all20 %>%ungroup() %>%  filter(.$follow==1934)
...
## DJL
#### Create timelag to last waypoints #### 
SUAQ_waypoints_11all20<- SUAQ_waypoints_11all20 %>% 
  group_by(follow)%>% arrange(manualTime,.by_group = TRUE) %>% 
  mutate(timelag=as.numeric(difftime(as.POSIXlt(manualTime),
         ...
         speed = manual_distTolast/timelag,
