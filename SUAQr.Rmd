...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% 
  mutate(manualTime=if_else((automaticTime%in% c(hms::as_hms("06:31:22")))&(follow==1501),hms::as_hms("11:30:00"),manualTime))
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% 
...
duplicates_datefocal <-duplicates_datefocal %>% unnest_wider(automaticDatetime) %>% 
  ...
  filter(!(is.na(automaticDatetime1)&is.na(automaticDatetime2)&is.na(automaticDatetime3))) # for three rows there is no automaticDatetime
...
##### Use !!sym(columnname) for using columnnames in functions where dplyr is used within

temp_SUAQ_waypoints_11all20_cleanFocalnames <- SUAQ_waypoints_11all20 %>% filter(focal==SUAQ_followlog.FocalName)
test<- own_valuefinder(df=temp_SUAQ_waypoints_11all20_cleanFocalnames,pk=1271,pkcol="identification",replaceterm="unk",startcol = "focal",targetcol="SUAQ_followlog.ClassFocal",lookuptype="temporally",datecol="manualDate")
df=SUAQ_waypoints_11all20
pk=29714
pkcol="identification"
replaceterm="unk"
startcol = "focal"
...
SUAQ_waypoints_11all20 <-
  SUAQ_waypoints_11all20 %>% ungroup() %>%  # replace any SexClass value which is unk or NA to the temporally nearest other Sex Class value for this orangutan
  rowwise() %>% 
  mutate(ClassFocal_ergaenzt = ifelse((is.na(SUAQ_followlog.ClassFocal)|(SUAQ_followlog.ClassFocal== "unk")),
                                  own_valuefinder(
                                          SUAQ_waypoints_11all20,
                                          identification,
                                          "identification",
...
SUAQ_waypoints_11all20 <-SUAQ_waypoints_11all20 %>% 
  mutate(ClassFocal_ergaenzt=if_else(is.na(ClassFocal_ergaenzt),.$SUAQ_followlog.ClassFocal,ClassFocal_ergaenzt)) %>% ungroup()
...
plot_gps_pttype <- SUAQ_waypoints_11all20[order(SUAQ_waypoints_11all20$manualTime , decreasing = TRUE ),] %>% 
  group_by(follow) %>%
  filter((follow %in% c(1501))) %>%
  ggplot()+
