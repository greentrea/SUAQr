...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>% 
  mutate(sym = if_else(sym=="Horn","Radio, Beacon",sym)) %>% 
  mutate(sym = if_else(sym=="Pin, Green","Flag, Green",sym))%>% 
  mutate(sym = if_else(sym=="Navaid, Green","Circle, Green",sym))%>% 
  mutate(sym = if_else(sym=="Navaid, Blue","Circle, Blue",sym))
...
temp<-SUAQ_waypoints_11all20 %>% 
  filter(follow %in% c("1101324")) 
temp<-SUAQ_waypoints_11all20 %>% 
  filter(fileN %in% c("20150204_Friska_Fikar_NN"))

plot_gps_pttype<-SUAQ_waypoints_11all20[order(SUAQ_waypoints_11all20$manualTime , decreasing = TRUE ),] %>% # reorder time 
  filter(follow %in% c("2047")) %>% 
  group_by(follow) %>%
  ggplot()+
  geom_point(aes(E,N,color=as.character(manualTime)),size=1)+
  geom_point(data = SUAQ_locationnetwork_loc,aes(E,N),size=2)+
