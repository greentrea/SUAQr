...
SUAQ_waypoints_11all20 <- SUAQ_waypoints_11all20 %>%
  ...
  mutate(follownr_reliability=if_else(!is.na(follow),"high","low")) %>%
...
test <- test %>% group_by(focal,manualDate,SUAQ_followlog.FocalName) %>% summarise(count=n())
test_df <- test %>% left_join(SUAQ_orangutans[,c(1,2,6)],by=c("focal"="SUAQ_orangutans.orangutanname","SUAQ_followlog.FocalName"="SUAQ_orangutans.Mother"),keep = FALSE,na_matches = "never",suffix=c("","infantMother."))
test_df <- test_df %>% left_join(SUAQ_orangutans[,c(1,2,6)],by=c("focal"="SUAQ_orangutans.Mother","SUAQ_followlog.FocalName"="SUAQ_orangutans.orangutanname"),keep = FALSE,na_matches = "never",suffix=c("","motherInfant."))
