...
levels(as.factor(SUAQ_followlog$GPS))
...
SUAQ_waypoints_11_20_wFN <- SUAQ_waypoints_11_20 %>%
  left_join(SUAQ_followlog,by=c("manualDate"="Date","orangutanname"="FocalName")) %>% 
...
SUAQ_waypointswithoutfollownumber<- SUAQ_waypoints_11_20_wFN %>%filter(is.na(FollowNumber)) %>% group_by(fileN) %>% summarise(count=n())
SUAQ_waypointswithfollownumber<- SUAQ_waypoints_11_20_wFN %>%filter(!is.na(FollowNumber)) %>% group_by(fileN) %>% summarise(count=n())
...
tmp<- SUAQ_waypoints_11_20 %>% filter(.$orangutanname=="robbie")
tmp0<- SUAQ_waypoints_11_20_wFN %>% filter(.$manualDate==as.Date("2014-02-19", tz = "Asia/Pontianak"))

tmp2<- SUAQ_waypoints_11_20_wFN %>% filter(.$fileN=="20130516_Islo_Mudin_FN")
tmp3<- SUAQ_followlog %>% filter(.$FocalName=="robbie")
tmp4<- SUAQ_followlog %>% filter(.$Date==as.Date("2014-03-08", tz = "Asia/Pontianak"))
